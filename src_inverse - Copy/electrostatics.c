...
#define rho_c_J(i,j,k) rho_c_J[(k)*nxp*nyp+(j)*nxp+(i)]
    ...
    for (den_i = 1; den_i < pSPARC->Nspden/2*2+1; den_i++) {
        for (i = 0; i < DMnd; i++) {
            pSPARC->electronDens_at[den_i*DMnd + i] = 0.0;
            pSPARC->electronDens_core[den_i*DMnd + i] = 0.0;
        }
    }
            ...
            for (k = 0; k < nz; k++) {
                ...
                for (j = 0; j < ny; j++) {
                    ...
                    for (i = 0; i < nx; i++) {
                        ...
                        assert(pSPARC->Nspden <= 2 && pSPARC->spin_typ <= 1);
                        for (den_i = 1; den_i < pSPARC->Nspden/2*2+1; den_i++) {
                            pSPARC->electronDens_at(den_i*DMnd,i_DM,j_DM,k_DM) += (0.5 + (1.5 - den_i) * spin_frac) * rho_J(ip,jp,kp);
                            pSPARC->electronDens_core(den_i*DMnd,i_DM,j_DM,k_DM) += 0.5 * rho_c_J(ip,jp,kp);
                        }
                    }
                }
            }
        ...
        for (k = 0; k < DMnz; k++) {
            for (j = 0; j < DMny; j++) {
                for (i = 0; i < DMnx; i++) {
                    ...
                    assert(pSPARC->Nspden <= 2 && pSPARC->spin_typ <= 1);
                    for (den_i = 1; den_i < pSPARC->Nspden/2*2+1; den_i++) 
                        pSPARC->electronDens_at(den_i*DMnd,i,j,k) *= scal_fac;

                    Nelectron_check += pSPARC->electronDens_at(0,i,j,k) * pSPARC->Intgwt_phi[count];
                    count++;                    
                }
            }
        }    
    ...
    if(pSPARC->spin_typ == 1 && pSPARC->elecgs_Count == 0) {
        if (pSPARC->CyclixFlag) {
            ...
            for (k = 0; k < DMnz; k++) {
                for (j = 0; j < DMny; j++) {
                    for (i = 0; i < DMnx; i++) {
                        int_rhoup += pSPARC->electronDens_at(DMnd,i,j,k) * pSPARC->Intgwt_phi[count];
                        int_rhodn += pSPARC->electronDens_at(2*DMnd,i,j,k) * pSPARC->Intgwt_phi[count];
                        count++;
                    }
                }
            }
    ...
    if(pSPARC->spin_typ == 2 && pSPARC->elecgs_Count == 0) {
        assert(pSPARC->Nspden <= 2 && pSPARC->spin_typ <= 1);
    }
...
void poisson_RHS(SPARC_OBJ *pSPARC, double *rhs) {
    ...
    if (pSPARC->INVERSE_FLAG==1){
        for (int i = 0; i < DMnd; i++) {
            rhs[i] = 4.0 * M_PI * (pSPARC->psdChrgDens[i] + pSPARC->rho_data_inverse[i]);
        }
    } else {
        for (int i = 0; i < DMnd; i++) {
...
void Calculate_elecstPotential(SPARC_OBJ *pSPARC) {
    ...
    if (pSPARC->CyclixFlag) {
        ...
        for (int i = 0; i < DMnd; i++) {
            ...
            if (pSPARC->INVERSE_FLAG){
                int_rho += pSPARC->rho_data_inverse[i];
            } else {
...
#define phi(i,j,k) phi[(k)*nx_phi*ny_phi+(j)*nx_phi+(i)]
	...
	double *rho_av = (double *)calloc(DMnZ, sizeof(double));
...
#define phi(i,j,k) phi[(k)*nx_phi*ny_phi+(j)*nx_phi+(i)]
	...
	double *rho_av = (double *)calloc(DMnX*DMnY, sizeof(double));
