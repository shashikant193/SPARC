...
void Vector2Norm(const double *Vec, const int len, double *vec_2norm, MPI_Comm comm)
{
    ...
    for(k = 0; k < len; k++)
        sqsum += Vec[k]*Vec[k];
    *vec_2norm = sqsum;
    int nproc; MPI_Comm_size(comm, &nproc);
    if (nproc > 1)
...
int unique(int *array, int n) {
    ...
    for (i = 0; i < n; i++) {
        for (j = i+1; j < n; j++) {
            if (array[i] == array[j]) {
                break; // Duplicate element found
            }
        }
        // when no more duplicate is found, append it to new array
        if (j == n) {
            array1[n_unique++] = array[i];
        }
    }
    // overwirte the unique array by the unique elements
    if (n_unique < n) {
...
/**
 * @brief Calculate the definite integral of a function f(x) using Simpson's rule.
 *
 * @param from Lower bound of the integration interval.
 * @param to Upper bound of the integration interval.
 * @param n Number of intervals in the integration interval.
 * @param func Function to be integrated.
 * @param extraData Extra data required by func.
 * @ref https://rosettacode.org/wiki/Numerical_integration#C
 * @return double
 */
double integral_simpson(
    double from, double to, double n, double (*func)(), DATA_INFO *extraData)
{
    double h = (to - from) / n;
    double sum1 = 0.0;
    double sum2 = 0.0;
    ...
    for(int i = 0; i < n; i++)
        sum1 += func(from + h * i + h / 2.0, extraData);

    for(int i = 1; i < n; i++)
        sum2 += func(from + h * i, extraData);

    return h / 6.0 * (func(from, extraData) + func(to, extraData) + 4.0 * sum1 + 2.0 * sum2);
}


/**
 * @brief Calculate the definite integral of a function f(x) using Simpson's
 *        rule in parallel.
 *
 * @param from Lower bound of the integration interval.
 * @param to Upper bound of the integration interval.
 ...
 * @param func Function to be integrated.
 * @param extraData Extra data required by func.
 * @param comm Communicator where this integral is evaluated in parallel.
 * @ref https://rosettacode.org/wiki/Numerical_integration#C
 * @return double
 */
double integral_simpson_paral(
    double from, double to, size_t n, double (*func)(), DATA_INFO *extraData,
    MPI_Comm comm)
{
    double h = (to - from) / n;
    double sum1 = 0.0;
    ...
    // the following sum will be done in parallel
    int size_comm = 0, rank_comm = -1;
    ...
    // assgin task
    size_t n_loc = block_decompose(n, size_comm, rank_comm);
    size_t i_start = block_decompose_nstart(n, size_comm, rank_comm);
    size_t i_end = i_start + n_loc - 1;
    ...
    i_start = max(i_start, 1); // exclude 0
    for(size_t i = i_start; i <= i_end; i++)
        sum2 += func(from + h * i, extraData);

    // sum up sum1 and sum2 over all processes
    double sbuf[2] = {sum1, sum2}, rbuf[2] = {0.0, 0.0};
    MPI_Allreduce(sbuf, rbuf, 2, MPI_DOUBLE, MPI_SUM, comm);
    sum1 = rbuf[0];
    sum2 = rbuf[1];
...
/*
@ brief: function to calculate the band gap based on eigenvalues
*/
void Calculate_BandGap(SPARC_OBJ *pSPARC) {
    if(pSPARC->spincomm_index < 0 || pSPARC->kptcomm_index < 0) return;
    int rank_spincomm, rank_bandcomm;
    MPI_Comm_rank(pSPARC->spincomm, &rank_spincomm);
    MPI_Comm_rank(pSPARC->bandcomm, &rank_bandcomm);

    int spn_disp, kpt_disp, spn_i, kpt;
    int ns_max, ns_min, indx_min, indx_max;
    double eigmin_g, eigmax_g, eigmin_loc, eigmax_loc;
    
    int Nk = pSPARC->Nkpts_kptcomm;
    int Ns = pSPARC->Nstates;
        
    for(spn_i = 0; spn_i < pSPARC->Nspin_spincomm; spn_i++) {
        spn_disp = spn_i*Nk*Ns;
        
        if(pSPARC->Nspin == 1) {
            ns_max = ceil(pSPARC->Nelectron/2) - 1;
            ns_min = ceil(pSPARC->Nelectron/2);
        } else {
            if (spn_i + pSPARC->spincomm_index == 0) {
                ns_max = ceil(pSPARC->Nelectron_up) - 1;
                ns_min = ceil(pSPARC->Nelectron_up);
            } else if(spn_i + pSPARC->spincomm_index == 1) {
                ns_max = ceil(pSPARC->Nelectron_dn) - 1;
                ns_min = ceil(pSPARC->Nelectron_dn);
            }         
        }

        eigmin_g = pSPARC->lambda_sorted[ns_min];
        eigmax_g = pSPARC->lambda_sorted[ns_max];
        ...
        for(kpt = 0; kpt < pSPARC->Nkpts_kptcomm; kpt++){
            kpt_disp = spn_disp + kpt*pSPARC->Nstates;
            if(pSPARC->lambda_sorted[kpt_disp + ns_min] < eigmin_g){
                eigmin_g = pSPARC->lambda_sorted[kpt_disp + ns_min];
                indx_min = kpt;
            }
            if(pSPARC->lambda_sorted[kpt_disp + ns_max] > eigmax_g){
                eigmax_g = pSPARC->lambda_sorted[kpt_disp + ns_max];
                indx_max = kpt;
            }
        }
       ...
       // }
        ...
        if(eigmax_loc == eigmax_g && pSPARC->bandcomm_index == 0 && rank_bandcomm == 0) {
            printf("Maximum occupied eigenvalue is %.15f which occurs at kpt [%f %f %f] \n",eigmax_g, pSPARC->k1_loc[indx_max]/tpiblx, pSPARC->k2_loc[indx_max]/tpibly,pSPARC->k3_loc[indx_max]/tpiblz);
        }

        if(eigmin_loc == eigmin_g && pSPARC->bandcomm_index == 0 && rank_bandcomm == 0) {
            printf("Minimum unoccupied eigenvalue is %.15f which occurs at kpt [%f %f %f] \n",eigmin_g, pSPARC->k1_loc[indx_min]/tpiblx, pSPARC->k2_loc[indx_min]/tpibly,pSPARC->k3_loc[indx_min]/tpiblz);
        }
...
void print_vector_real(double *vec, int M, int N, char *fname)
{
...
void print_vector_complex(double _Complex *vec, int M, int N, char *fname)
{
    FILE *fp = fopen(fname, "w");
    fprintf(fp, "%d %d\n", M, N);
    for (int i = 0; i < M*N; i++) {
