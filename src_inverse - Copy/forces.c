...
void Calculate_nonlocal_forces(SPARC_OBJ *pSPARC)
{
    if (pSPARC->isGammaPoint) {
        if (pSPARC->CS_Flag == 1) {
            Calculate_nonlocal_forces_linear_CS(pSPARC);    
        ...
        #ifdef SPARCX_ACCEL
            if (pSPARC->useACCEL == 1 && pSPARC->cell_typ < 20 && pSPARC->SQ3Flag == 0 && pSPARC->Nd_d_dmcomm == pSPARC->Nd)
            {
...
//                     double *x_ptr = Psi + spn_i * size_s + n * DMnd;
...
//                     }
//                 }
//                 cblas_dgemm(CblasColMajor, CblasTrans, CblasNoTrans, pSPARC->nlocProj[ityp].nproj, ncol, ndc, pSPARC->dV, 
//                 	pSPARC->nlocProj[ityp].Chi[iat], ndc, x_rc, ndc, 1.0, beta+pSPARC->IP_displ[atom_index]*ncol, 
...
void Calculate_nonlocal_forces_linear_CS(SPARC_OBJ *pSPARC)
{
    ...
    // int n_end_local   = min(pSPARC->band_end_indx, Ns-1) - pSPARC->band_start_indx;
    //nonlocal_forces_intPsiTChi(pSPARC, alpha_2, pSPARC->Yorb, DMnd, ncol, 0, ncol-1); // TODO: set correct nstart and nend
    t1 = MPI_Wtime();
...
/**
 * @brief: function to convert Hessian along lattice directions to cartesian gradients
 */
void nonCart2Cart_hess(SPARC_OBJ *pSPARC, double *xx, double *xy, double *xz,
    double *yx, double *yy, double *yz, double *zx, double *zy, double *zz) 
{
#define T(i,j) T[i*3+j-4]
    const double *T = pSPARC->gradT;
    // find H_cart = T' * H * T
    double h11, h22, h33, h12, h21, h13, h31, h23, h32;
    h11 = T(1,1)*(*xx)*T(1,1) + T(1,1)*(*xy)*T(2,1) + T(1,1)*(*xz)*T(3,1)
        + T(2,1)*(*yx)*T(1,1) + T(2,1)*(*yy)*T(2,1) + T(2,1)*(*yz)*T(3,1)
        + T(3,1)*(*zx)*T(1,1) + T(3,1)*(*zy)*T(2,1) + T(3,1)*(*zz)*T(3,1);
    h22 = T(1,2)*(*xx)*T(1,2) + T(1,2)*(*xy)*T(2,2) + T(1,2)*(*xz)*T(3,2)
        + T(2,2)*(*yx)*T(1,2) + T(2,2)*(*yy)*T(2,2) + T(2,2)*(*yz)*T(3,2)
        + T(3,2)*(*zx)*T(1,2) + T(3,2)*(*zy)*T(2,2) + T(3,2)*(*zz)*T(3,2);
    h33 = T(1,3)*(*xx)*T(1,3) + T(1,3)*(*xy)*T(2,3) + T(1,3)*(*xz)*T(3,3)
        + T(2,3)*(*yx)*T(1,3) + T(2,3)*(*yy)*T(2,3) + T(2,3)*(*yz)*T(3,3)
        + T(3,3)*(*zx)*T(1,3) + T(3,3)*(*zy)*T(2,3) + T(3,3)*(*zz)*T(3,3);
    h12 = T(1,1)*(*xx)*T(1,2) + T(1,1)*(*xy)*T(2,2) + T(1,1)*(*xz)*T(3,2)
        + T(2,1)*(*yx)*T(1,2) + T(2,1)*(*yy)*T(2,2) + T(2,1)*(*yz)*T(3,2)
        + T(3,1)*(*zx)*T(1,2) + T(3,1)*(*zy)*T(2,2) + T(3,1)*(*zz)*T(3,2);
    h21 = T(1,2)*(*xx)*T(1,1) + T(1,2)*(*xy)*T(2,1) + T(1,2)*(*xz)*T(3,1)
        + T(2,2)*(*yx)*T(1,1) + T(2,2)*(*yy)*T(2,1) + T(2,2)*(*yz)*T(3,1)
        + T(3,2)*(*zx)*T(1,1) + T(3,2)*(*zy)*T(2,1) + T(3,2)*(*zz)*T(3,1);
    h13 = T(1,1)*(*xx)*T(1,3) + T(1,1)*(*xy)*T(2,3) + T(1,1)*(*xz)*T(3,3)
        + T(2,1)*(*yx)*T(1,3) + T(2,1)*(*yy)*T(2,3) + T(2,1)*(*yz)*T(3,3)
        + T(3,1)*(*zx)*T(1,3) + T(3,1)*(*zy)*T(2,3) + T(3,1)*(*zz)*T(3,3);
    h31 = T(1,3)*(*xx)*T(1,1) + T(1,3)*(*xy)*T(2,1) + T(1,3)*(*xz)*T(3,1)
        + T(2,3)*(*yx)*T(1,1) + T(2,3)*(*yy)*T(2,1) + T(2,3)*(*yz)*T(3,1)
        + T(3,3)*(*zx)*T(1,1) + T(3,3)*(*zy)*T(2,1) + T(3,3)*(*zz)*T(3,1);
    h23 = T(1,2)*(*xx)*T(1,3) + T(1,2)*(*xy)*T(2,3) + T(1,2)*(*xz)*T(3,3)
        + T(2,2)*(*yx)*T(1,3) + T(2,2)*(*yy)*T(2,3) + T(2,2)*(*yz)*T(3,3)
        + T(3,2)*(*zx)*T(1,3) + T(3,2)*(*zy)*T(2,3) + T(3,2)*(*zz)*T(3,3);
    h32 = T(1,3)*(*xx)*T(1,2) + T(1,3)*(*xy)*T(2,2) + T(1,3)*(*xz)*T(3,2)
        + T(2,3)*(*yx)*T(1,2) + T(2,3)*(*yy)*T(2,2) + T(2,3)*(*yz)*T(3,2)
        + T(3,3)*(*zx)*T(1,2) + T(3,3)*(*zy)*T(2,2) + T(3,3)*(*zz)*T(3,2);
    *xx = h11; *xy = h12; *xz = h13;
    *yx = h21; *yy = h22; *yz = h23;
    *zx = h31; *zy = h32; *zz = h33;
#undef T
}
