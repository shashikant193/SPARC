...
void scf(SPARC_OBJ *pSPARC)
{
    ...
    int NspdenDMnd = pSPARC->Nspden * DMnd;
    int i;
    ...
    if (pSPARC->MixingVariable == 1)  { // potential mixing
        if (pSPARC->BC == 2 || pSPARC->BC == 0) {
            VectorSum(pSPARC->Veff_loc_dmcomm_phi, NspdenDMnd, &veff_mean, pSPARC->dmcomm_phi);
            veff_mean /= ((double) (pSPARC->Nd * pSPARC->Nspden));
        }
    }
...
void scf_loop(SPARC_OBJ *pSPARC) {
    ...
    int DMnd = pSPARC->Nd_d;
    int NspdenDMnd = pSPARC->Nspden * DMnd;
    int sindx_rho = (pSPARC->Nspden == 2) ? DMnd : 0;
    ...
    while (SCFcount < pSPARC->MAXIT_SCF) {
        ...
        if (pSPARC->MixingVariable == 1) { // potential mixing
            ...
            if (pSPARC->BC == 2 || pSPARC->BC == 0) {
                VectorShift(pSPARC->Veff_loc_dmcomm_phi_in, NspdenDMnd, veff_mean, pSPARC->dmcomm_phi);
            }

            Escc = Calculate_Escc(
                pSPARC, NspdenDMnd, pSPARC->Veff_loc_dmcomm_phi, 
        ...
        if (pSPARC->MixingVariable == 1 && pSPARC->BC == 2) { // potential mixing 
            ...
            VectorShift(pSPARC->Veff_loc_dmcomm_phi, NspdenDMnd, -veff_mean, pSPARC->dmcomm_phi);
            pSPARC->veff_mean = veff_mean;
        }
...
{
    if (pSPARC->dmcomm_phi == MPI_COMM_NULL) return;
    unsigned i, den_i, dindx;
    for (den_i = 0; den_i < pSPARC->Nspden; den_i++) {
        dindx = den_i * pSPARC->Nd_d;
        for (i = 0; i < pSPARC->Nd_d; i++) {
            pSPARC->Veff_loc_dmcomm_phi[dindx + i] = pSPARC->XCPotential[dindx + i] + pSPARC->elecstPotential[i];
        }
    }
}
...
void Update_mixing_hist_xk(SPARC_OBJ *pSPARC, double veff_mean) 
{
    ...
    if (pSPARC->spin_typ == 0) {
        ...
        // TODO: here need to change for non-collinear
