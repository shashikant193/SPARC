    ...
    ATOM_NLOC_INFLUENCE_OBJ **Atom_Influence_nloc, int *DMVertices, MPI_Comm comm)
{
    ...
    double range_x = pSPARC->range_x;
    double range_y = pSPARC->range_y;
    double range_z = pSPARC->range_z;
    double Ox = cell_orig[0];
    double Oy = cell_orig[1];
    double Oz = cell_orig[2];
    Lx = cell[0];
    Ly = cell[1];
    Lz = cell[2];
    int BCx = BCs[0];
    int BCy = BCs[1];
    int BCz = BCs[2];
    // extra buffer for deciding which images to include, if range_x extends beyond Lx
    double buf_xl = Ox; buf_xl = max(buf_xl, 0.0);
    double buf_yl = Oy; buf_yl = max(buf_yl, 0.0);
    double buf_zl = Oz; buf_zl = max(buf_zl, 0.0);
    double buf_xr = range_x - Ox - Lx; buf_xr = max(buf_xr, 0.0);
    double buf_yr = range_y - Oy - Ly; buf_yr = max(buf_yr, 0.0);
    double buf_zr = range_z - Oz - Lz; buf_zr = max(buf_zr, 0.0);
    ...
    *Atom_Influence_nloc = (ATOM_NLOC_INFLUENCE_OBJ *)malloc(sizeof(ATOM_NLOC_INFLUENCE_OBJ) * pSPARC->Ntypes);
    ...
    for (ityp = 0; ityp < pSPARC->Ntypes; ityp++) {
        ...
        for (i = 0; i < pSPARC->nAtomv[ityp]; i++) {
            ...
            // get relative atom positions in the shifted cell
            double x0_shift = x0 - Ox;
            double y0_shift = y0 - Oy;
            double z0_shift = z0 - Oz;
            atmcount++;
            ...
            if (BCx == 0) {
                if (pSPARC->SQFlag == 1) {
                    ...
                    ppmax = floor((rcbox_x + Lx - x0_shift + rcut_x + buf_xr) / Lx + TEMP_TOL);
                    ppmin = -floor((rcbox_x + x0_shift + rcut_x + buf_xl) / Lx + TEMP_TOL);
                } else {
                    ppmax = floor((rcbox_x + Lx - x0_shift + buf_xr) / Lx + TEMP_TOL);
                    ppmin = -floor((rcbox_x + x0_shift + buf_xl) / Lx + TEMP_TOL);
                }
            }
            if (BCy == 0) {
                if (pSPARC->SQFlag == 1) {
                    double rcut_y = pSPARC->pSQ->nloc[1] * pSPARC->delta_y;
                    qqmax = floor((rcbox_y + Ly - y0_shift + rcut_y + buf_yr) / Ly + TEMP_TOL);
                    qqmin = -floor((rcbox_y + y0_shift + rcut_y + buf_yl) / Ly + TEMP_TOL);
                } else {
                    qqmax = floor((rcbox_y + Ly - y0_shift + buf_yr) / Ly + TEMP_TOL);
                    qqmin = -floor((rcbox_y + y0_shift + buf_yl) / Ly + TEMP_TOL);
                }
            }
            if (BCz == 0) {
                if (pSPARC->SQFlag == 1) {
                    double rcut_z = pSPARC->pSQ->nloc[2] * pSPARC->delta_z;
                    rrmax = floor((rcbox_z + Lz - z0_shift + rcut_z + buf_zr) / Lz + TEMP_TOL);
                    rrmin = -floor((rcbox_z + z0_shift + rcut_z + buf_zl) / Lz + TEMP_TOL);
                } else {
                    rrmax = floor((rcbox_z + Lz - z0_shift + buf_zr) / Lz + TEMP_TOL);
                    rrmin = -floor((rcbox_z + z0_shift + buf_zl) / Lz + TEMP_TOL);
                }
            }
...
/**
 * @brief   Find the list of all atoms that influence the processor
 *          domain in psi-domain. Using the global cell and BCs.
 */
void GetInfluencingAtoms_nloc(
    SPARC_OBJ *pSPARC, ATOM_NLOC_INFLUENCE_OBJ **Atom_Influence_nloc, int *DMVertices, MPI_Comm comm)
{
    double cell[3];
    cell[0] = pSPARC->range_x;
    cell[1] = pSPARC->range_y;
    cell[2] = pSPARC->range_z;
    double cell_orig[3] = {0.0, 0.0, 0.0};
    int BCs[3];
    BCs[0] = pSPARC->BCx;
    BCs[1] = pSPARC->BCy;
    BCs[2] = pSPARC->BCz;
    GetInfluencingAtoms_nloc_kernel(
        pSPARC, cell_orig, cell, BCs, Atom_Influence_nloc, DMVertices, comm);
}
        ...
        ATOM_NLOC_INFLUENCE_OBJ *Atom_Influence_nloc, int *DMVertices, MPI_Comm comm)
{
    ...
    if (pSPARC->CyclixFlag) {
     ...
     if (pSPARC->isGammaPoint && pSPARC->spDFT_Flag == 1) {
	} else {
    	(*nlocProj) = (NLOC_PROJ_OBJ *)malloc( sizeof(NLOC_PROJ_OBJ) * pSPARC->Ntypes ); // TODO: deallocate!!
	}
