...
void collect_Veff_global(SPARC_OBJ *pSPARC, double *Veff_global)
{
    ...
    if (proc_active == 1) {
        ...
        D2D(&d2d_sender, &d2d_recvr, gridsizes, pSPARC->DMVertices_dmcomm, pSPARC->Veff_loc_dmcomm,
            rDMVert, Veff_global, pSPARC->dmcomm, sdims, recv_comm, rdims, pSPARC->bandcomm, sizeof(double));
    ...
    double *x0, int count, int kpt, int spn_i)
{
    ...
    double *v_tilde_f = (double *)malloc(size_k * sizeof(*v_tilde_f)); // filtered basis
    ...
    ChebyshevFiltering(ESPRC_k, E_k->DMVert_ex, E_k->v_tilde, nrow, 
                       v_tilde_f, nrow, ncol, m, lambda_cutoff,
    ...
    assert(ESPRC_k->Xorb_BLCYC != NULL);
    if (ESPRC_k->npband > 1 || ESPRC_k->Nspinor_eig != ESPRC_k->Nspinor_spincomm) {
        ESPRC_k->Yorb_BLCYC = (double *) malloc(ESPRC_k->nr_orb_BLCYC * ESPRC_k->nc_orb_BLCYC * sizeof(double));
        assert(ESPRC_k->Yorb_BLCYC != NULL);
    }

    // project the element Hamiltonian H_k onto the basis
    Project_Hamiltonian(ESPRC_k, E_k->DMVert_ex, v_tilde_f, nrow, E_k->v_tilde, nrow, 
...
{    
    ...
    // int nrow_ex = E_k->nd_ex_d;
    // orthChol(ESPRC_k, E_k->nd_ex, E_k->nALB, E_k->v_tilde,
    ...
    orthChol(ESPRC_k, E_k->nd, E_k->nALB, E_k->v,
    ...
    int isInitSCF, int kpt, int spn_i)
{
    ...
    for (int k = 0; k < DDBP_info->n_elem_elemcomm; k++) {
        ...
        int size_k = nrow * ncol;
        if (isGammaPoint) {
            double *v = E_k->v + kpt*size_k;
            double *v_prev = E_k->v_prev + kpt*size_k;
            memcpy(v_prev, v, size_k * sizeof(*v));
        } else {
            double _Complex *v = E_k->v_cmplx + kpt*size_k;
            double _Complex *v_prev = E_k->v_prev_cmplx + kpt*size_k;
            memcpy(v_prev, v, size_k * sizeof(*v));
        }
    }
}
...
{    
    ...
    for (int k = 0; k < DDBP_info->n_elem_elemcomm; k++) {
        ...
        double *v_i = E_k->v + kpt * size_k;
        double *v_im1 = E_k->v_prev + kpt * size_k;
