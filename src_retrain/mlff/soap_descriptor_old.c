...
void build_nlist(const double rcut, const int nelem, const int natom, const double * const atompos,
	...
	for (int i = 0; i < natom_domain; i++){
		...
		// if (i==0) printf("came here!  (img_py+img_ny) * L2: %.15f, 2*M_PI: %.15f, (img_py+img_ny) * L2 - 2*M_PI: %.15f\n", (img_py+img_ny) * L2, 2*M_PI, (img_py+img_ny) * L2 - 2*M_PI);
...
void build_soapObj(SoapObj *soap_str, NeighList *nlist, double* rgrid, double* h_nl, double* dh_nl, double *atompos, int Nmax, int Lmax, double beta_3, double xi_3, int N_rgrid) {
	...
	for (int i = 0; i < soap_str->natom_domain; i++){
		...
		for (int j = 0; j < nlist->Nneighbors[i]; j++){
			...
			// printf("dr: %f, i: %d, j: %d, imgx_temp[j]: %d %d %d\n", dr, i, j, imgx_temp[j], imgy_temp[j], imgz_temp[j]);
			if (fabs(dr) < 1.0E-15){
	...
	// FILE *fp;
	// fp = fopen("cnlm.txt","w");
	// fprintf(fp,"Counters: %d %d %d\n", counter1, counter2, counter3);

	// for (int n=0; n < Nmax; n++){
	// 	for (int l=0; l < Lmax+1; l++){
	// 		for (int m=0; m < l+1; m++){
	// 			int idx1 = (n*(Lmax+1)*(Lmax+2))/2 + (l*(l+1))/2 + m;
	// 			fprintf(fp, "%d %d %d %.15f+%.15fi    %.15f+%.15fi   %.15f+%.15fi\n", n, l, m, creal(cnlm[0][0][idx1]), cimag(cnlm[0][0][idx1]), creal(dcnlm_dX[0][0][10][idx1]), cimag(dcnlm_dX[0][0][10][idx1]), creal(dcnlm_dY[0][0][10][idx1]), cimag(dcnlm_dY[0][0][10][idx1]));
	// 			// fprintf(fp, "%d %d %d   %.15f+%.15fi   %.15f+%.15fi\n", n, l, m, creal(dcnlm_dX[0][0][10][idx1]), cimag(dcnlm_dX[0][0][10][idx1]), creal(dcnlm_dY[0][0][10][idx1]), cimag(dcnlm_dY[0][0][10][idx1]));
	...
	FILE *fptr;
	fptr = fopen("descriptors_test.txt","w");
	if(fptr == NULL)
   {
      printf("Error opening the descriptor.txt file!");   
      exit(1);
    ...
    for( int j = 0; j < nlist->unique_Nneighbors[0]+1; j++){
		fprintf(fptr,"\n");
    	fprintf(fptr,"%s\n","********************************************************");
   		fprintf(fptr,"Neighbor %d \n", j);
   		fprintf(fptr,"%s\n","********************************************************");
   		fprintf(fptr,"\n");
   		for (int i = 0; i < soap_str->size_X3; i++){
   			fprintf(fptr, "%10.15f, %10.15f, %10.15f, %10.15f", soap_str->X3[0][i], soap_str->dX3_dX[0][j][i], soap_str->dX3_dY[0][j][i], soap_str->dX3_dZ[0][j][i]);
   			fprintf(fptr, "\n");
   		}
	}
   ...
   for (int i = 0; i < soap_str->size_X3; i++){
   	for (int j = 0; j < 6; j++){
   		fprintf(fptr, "%10.15f ", soap_str->dX3_dF[0][j][i]);
   	}
   	fprintf(fptr, "\n");
   }
