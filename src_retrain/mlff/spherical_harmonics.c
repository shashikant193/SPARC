...
void sph_harmonics_sphericart(const double x, const double y, const double z, const int l_max,
				double * const sph, double * const dsph,
				double * const ddsph)
{	
	int sph_size = (l_max + 1) * (l_max + 1);
	int dsph_size = 3*(l_max + 1) * (l_max + 1);
	int ddsph_size = 9*(l_max + 1) * (l_max + 1);
	double xyz[3] = {x, y, z};
	sphericart_calculator_t* calculator = sphericart_new(l_max, 1);
...
void real_to_complex_sph_harmonics(double * const sph_real, double * const dsph_real, double * const ddsph_real, double complex *sph, 
												double complex *dsph, double complex *ddsph, int L_max)
{	

	for (int l = 0; l < L_max+1; l++){
			sph[YR(l,0)] = sph_real[YR(l,0)] + 0.0*I;
		for (int m = 1; m < l+1; m++){
			
			sph[YR(l,m)] = pow(-1.0, m) *(sph_real[YR(l,m)] +  I*sph_real[YR(l,-m)])/sqrt(2);
			sph[YR(l,-m)] = (sph_real[YR(l,m)] -  I*sph_real[YR(l,-m)])/sqrt(2);
	...
	for (int dim = 0; dim < 3; dim++){
		int start_idx = dim *(L_max+1)*(L_max+1);
		for (int l = 0; l < L_max+1; l++){
			dsph[start_idx+YR(l,0)] = dsph_real[start_idx+YR(l,0)] + 0.0*I;
			for (int m = 1; m < l+1; m++){
				dsph[start_idx+YR(l,m)] = pow(-1.0, m) *(dsph_real[start_idx+YR(l,m)] +  I*dsph_real[start_idx+YR(l,-m)])/sqrt(2);
				dsph[start_idx+YR(l,-m)] = (dsph_real[start_idx+YR(l,m)] -  I*dsph_real[start_idx+YR(l,-m)])/sqrt(2);
			}
		}
	}
	...
	for (int dim = 0; dim < 9; dim++){
		int start_idx = dim *(L_max+1)*(L_max+1);
		for (int l = 0; l < L_max+1; l++){
			ddsph[start_idx+YR(l,0)] = ddsph_real[start_idx+YR(l,0)] + 0.0*I;
			for (int m = 1; m < l+1; m++){
				ddsph[start_idx+YR(l,m)] = pow(-1.0, m) *(ddsph_real[start_idx+YR(l,m)] +  I*ddsph_real[start_idx+YR(l,-m)])/sqrt(2);
				ddsph[start_idx+YR(l,-m)] = (ddsph_real[start_idx+YR(l,m)] -  I*ddsph_real[start_idx+YR(l,-m)])/sqrt(2);
			}
		}
	}
