    ...
    int band_start_indx_source, double *pres_exx)
{
    ...
    for (loop = 0; loop < NL; loop ++) {
        ...
        poissonSolve(pSPARC, rhs, pSPARC->pois_const_press, count-base, DMnd, dims, phi, comm);
    ...
    double _Complex *psi, int ldp, double *occ, int Nband_source, int band_start_indx_source, int kpt_q, double *pres_exx)
{
    ...
    for (loop = 0; loop < NL; loop ++) {
        ...
        // Solve all Poisson's equation 
        poissonSolve_kpt(pSPARC, rhs, pSPARC->pois_const_press, count-base, DMnd, dims, phi, kpt_k_list, kpt_q_list, comm);
