...
void Calc_DX_kpt_orig(
    ...
    const double _Complex phase_fac_r_m,
...
)
{
    ...
    double _Complex phase_fac_l_m = conj(phase_fac_r_m);
    int sz = 2 * (m_epos - m_spos) * radius;
    double _Complex *Phase_fac = (double _Complex *) malloc(sz * sizeof(double _Complex));
...
#pragma omp simd
            ...
            for (i = 0; i < niters; i++)
            {
                ...
                double _Complex temp = X[ishift_X] * c;
                for (r = 1; r <= radius; r++)
                {
