...
void GaussQuadrature(SPARC_OBJ *pSPARC, int SCFCount) {
    ...
    restrict_to_subgrid(pSQ->Veff_loc_SQ, pSQ->Veff_PR,
        DMnx_PR, DMnx, DMnxny_PR, DMnxny, 
        nloc[0], nloc[0]+DMnx-1, nloc[1], nloc[1]+DMny-1, nloc[2], nloc[2]+DMnz-1, 
        0, 0, 0);
    ...
    double rho_pc, *rho;
    rho = (double *) malloc(sizeof(double) * DMnd);
 ...
 */
void LanczosAlgorithm_gauss(SPARC_OBJ *pSPARC, double *vkm1, double *lambda_min, double *lambda_max, int nd) {
    int rank, i;
    ...
    int le_count = 0;
    double *lanczos_vec = (pSPARC->SQ_gauss_mem == 1) ? pSQ->lanczos_vec_all[nd] : pSQ->lanczos_vec;
...
void TridiagEigenSolve_gauss(SPARC_OBJ *pSPARC, double *diag, double *subdiag, int nd, double *lambda_min, double *lambda_max) {
    ...
    double *w = (pSPARC->SQ_gauss_mem == 1) ? pSQ->w_all[nd] : pSQ->w;
    int count = 0;
