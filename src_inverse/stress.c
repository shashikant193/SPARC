...
void Calculate_electronic_stress_linear(SPARC_OBJ *pSPARC) {
    ...
    t1 = MPI_Wtime();
    Calculate_XC_stress_linear(pSPARC);
    if (pSPARC->ixc[2]) {
...
*/
void Calculate_XC_stress_nlcc_linear(SPARC_OBJ *pSPARC, double *stress_xc_nlcc) {
...
* @brief: find stress contributions from exchange-correlation
*/
void Calculate_XC_stress_linear(SPARC_OBJ *pSPARC) {
    if (pSPARC->dmcomm_phi == MPI_COMM_NULL) return;
    int rank;
    MPI_Comm_rank(pSPARC->dmcomm_phi, &rank);
    ...
    if(strcmpi(pSPARC->XC,"LDA_PW") == 0 || strcmpi(pSPARC->XC,"LDA_PZ") == 0){
        ...
        pSPARC->stress_xc[1] = pSPARC->stress_xc[2] = pSPARC->stress_xc[4] = 0.0;
        int DMnd, i;
        DMnd = (pSPARC->Nspden/2*2+1) * pSPARC->Nd_d;
        ...
        Gradient_vectors_dir(pSPARC, pSPARC->Nd_d, pSPARC->DMVertices, (pSPARC->Nspden/2*2+1), 0.0, rho, Drho_x, 0, pSPARC->dmcomm_phi);
        Gradient_vectors_dir(pSPARC, pSPARC->Nd_d, pSPARC->DMVertices, (pSPARC->Nspden/2*2+1), 0.0, rho, Drho_y, 1, pSPARC->dmcomm_phi);
        Gradient_vectors_dir(pSPARC, pSPARC->Nd_d, pSPARC->DMVertices, (pSPARC->Nspden/2*2+1), 0.0, rho, Drho_z, 2, pSPARC->dmcomm_phi);
    ...
    if (pSPARC->NLCC_flag) {
        ...
        Calculate_XC_stress_nlcc_linear(pSPARC, stress_xc_nlcc);
        for(int i = 0; i < 6; i++)
    ...
    if (pSPARC->ixc[3] != 0) { // either vdW_DF1 or vdW_DF2, compute the contribution of nonlinear correlation of vdWDF on stress
        if (pSPARC->spin_typ == 0) {
...
#ifdef SPARCX_ACCEL
            if (pSPARC->useACCEL == 1 && pSPARC->cell_typ < 20 && pSPARC->Nd_d_dmcomm == pSPARC->Nd && pSPARC->SQ3Flag == 0)
            {
...
void Calculate_nonlocal_kinetic_stress_linear(SPARC_OBJ *pSPARC)
{
        ...
        for (ityp = 0; ityp < pSPARC->Ntypes; ityp++) {
            ...
            for (iat = 0; iat < pSPARC->Atom_Influence_nloc[ityp].n_atom; iat++) {
                ...
                if (pSPARC->SQ3Flag == 1){
                    ...
                    cblas_dgemm(CblasColMajor, CblasTrans, CblasNoTrans, pSPARC->nlocProj[ityp].nproj, ncol, ndc, pSPARC->dV, pSPARC->nlocProj[ityp].Chi[iat], ndc, 
                                psi_rc, ndc, 1.0, beta2+pSPARC->IP_displ[atom_index]*ncol, pSPARC->nlocProj[ityp].nproj); // multiply dV to get inner-product
                }
    ...
    for(spn_i = 0; spn_i < nspin; spn_i++) {
        for (dim = 0; dim < 3; dim++) {
            ...
            if(dim == 0){
                ...
                double *dpsi_1p = YorZ; 
                if (pSPARC->SQ3Flag == 1) {
                    Gradient_vectors_dir(pSPARC, DMnd, pSPARC->DMVertices_dmcomm, ncol, 0.0, pSPARC->Xorb+spn_i*size_s, dpsi_full, 0, pSPARC->dmcomm);
                    dpsi_1p = dpsi_full;
                }
                for(n = 0; n < ncol; n++){
                    dpsi_ptr = YorZ + n * DMnd; // dpsi_1
                    dpsi_ptr2 = dpsi_1p + n * DMnd; // dpsi_1p
                    dpsi1_dpsi1 = 0.0;
                    for(i = 0; i < DMnd; i++){
                        dpsi1_dpsi1 += *(dpsi_ptr + i) * *(dpsi_ptr2 + i);
                    }
                    g_nk = (pSPARC->SQ3Flag == 0) ? pSPARC->occ[spn_i*Ns + n + pSPARC->band_start_indx] : 1.0;
                    stress_k[0] += dpsi1_dpsi1 * g_nk; // component (1,1)
                }
                ...
                for(n = 0; n < ncol; n++){
                    ...
                    g_nk = (pSPARC->SQ3Flag == 0) ? pSPARC->occ[spn_i*Ns + n + pSPARC->band_start_indx] : 1.0;
                    stress_k[1] += dpsi1_dpsi2 * g_nk; // component (1,2)
                }
                ...
                for(n = 0; n < ncol; n++){
                    ...
                    g_nk = (pSPARC->SQ3Flag == 0) ? pSPARC->occ[spn_i*Ns + n + pSPARC->band_start_indx] : 1.0;
                    stress_k[2] += dpsi1_dpsi3 * g_nk; // component (1,3)
                }
            } else if(dim == 1){
                // component (2,3)
                double *dpsi_3p; 
                ...
                for(n = 0; n < ncol; n++){
                    ...
                    g_nk = (pSPARC->SQ3Flag == 0) ? pSPARC->occ[spn_i*Ns + n + pSPARC->band_start_indx] : 1.0;
                    stress_k[4] += dpsi2_dpsi3 * g_nk; // component (2,3)
                }
                ...
                for(n = 0; n < ncol; n++){
                    dpsi_ptr = YorZ + n * DMnd; // dpsi_1
                    dpsi_ptr2 = dpsi_2p + n * DMnd; // dpsi_1p
                    dpsi2_dpsi2 = 0.0;
                    for(i = 0; i < DMnd; i++){
                        dpsi2_dpsi2 += *(dpsi_ptr + i) * *(dpsi_ptr2 + i);
                    }
                    g_nk = (pSPARC->SQ3Flag == 0) ? pSPARC->occ[spn_i*Ns + n + pSPARC->band_start_indx] : 1.0;
                    stress_k[3] += dpsi2_dpsi2 * g_nk; // component (2,2)
                }
            } else if (dim == 2) {
                // component (3,3)
                double *dpsi_3p = YorZ; 
                ...
                for(n = 0; n < ncol; n++){
                    dpsi_ptr = YorZ + n * DMnd; // dpsi_1
                    dpsi_ptr2 = dpsi_3p + n * DMnd; // dpsi_1p
                    dpsi3_dpsi3 = 0.0;
                    for(i = 0; i < DMnd; i++){
                        dpsi3_dpsi3 += *(dpsi_ptr + i) * *(dpsi_ptr2 + i);
                    }
                    g_nk = (pSPARC->SQ3Flag == 0) ? pSPARC->occ[spn_i*Ns + n + pSPARC->band_start_indx] : 1.0;
                    stress_k[5] += dpsi3_dpsi3 * g_nk; // component (3,3)
                }
            }
        }    
        count++;
    ...
    stress_k[0] *= -(2.0/pSPARC->Nspin); // component (1,1)
    stress_k[1] *= -(2.0/pSPARC->Nspin); // component (1,2)
    stress_k[2] *= -(2.0/pSPARC->Nspin); // component (1,3)
    stress_k[3] *= -(2.0/pSPARC->Nspin); // component (2,2)
    stress_k[4] *= -(2.0/pSPARC->Nspin); // component (2,3)
    stress_k[5] *= -(2.0/pSPARC->Nspin); // component (3,3)
    ...
    if (pSPARC->npNd > 1) {
        ...
        if (pSPARC->SQ3Flag == 1) {
            MPI_Allreduce(MPI_IN_PLACE, alpha2, pSPARC->IP_displ[pSPARC->n_atom] * ncol * nspin, MPI_DOUBLE, MPI_SUM, pSPARC->dmcomm);
        }
    }
        ...
        for (ityp = 0; ityp < pSPARC->Ntypes; ityp++) {
            ...
            for (iat = 0; iat < pSPARC->nAtomv[ityp]; iat++) {
                ...
                for (n = pSPARC->band_start_indx; n <= pSPARC->band_end_indx; n++) {
                    g_nk = (pSPARC->SQ3Flag == 0) ? pSPARC->occ[spn_i*Ns + n] : 1.0;
    ...
    // for ext-FPMD method, add kinetic term (diag terms only) for high-energy electrons
    if (pSPARC->ext_FPMD_Flag != 0) {
        double highE_Tk = pSPARC->ext_FPMD_highETk;
        pSPARC->stress_k[0] -= 2.0 / 3.0 * highE_Tk; // component (1,1)
        pSPARC->stress_k[3] -= 2.0 / 3.0 * highE_Tk; // component (2,2)
        pSPARC->stress_k[5] -= 2.0 / 3.0 * highE_Tk; // component (3,3)
    }
    ...
    if (pSPARC->SQ3Flag == 1) free(alpha2);
    free(alpha);
    free(stress_k);
    free(stress_nl);
    free(SJ);
    ...
    for(spn_i = 0; spn_i < nspin; spn_i++) {
        for(kpt = 0; kpt < pSPARC->Nkpts_kptcomm; kpt++) {
            ...
            for (dim = 0; dim < 3; dim++) {
                ...
                // find dPsi in direction dim
                Gradient_vectors_dir_kpt(pSPARC, DMnd, pSPARC->DMVertices_dmcomm, 
                    ncol, 0.0, pSPARC->Xorb_kpt+spn_i*size_s+kpt*size_k, 
                    pSPARC->Yorb_kpt, dim, &kpt_vec, pSPARC->dmcomm);
                ...
                if(dim == 0){
                    ...
                    Gradient_vectors_dir_kpt(pSPARC, DMnd, pSPARC->DMVertices_dmcomm, 
                        ncol, 0.0, pSPARC->Xorb_kpt+spn_i*size_s+kpt*size_k, 
                        dpsi_full, 1, &kpt_vec, pSPARC->dmcomm);
                    //ts = MPI_Wtime();
                    ...
                    Gradient_vectors_dir_kpt(pSPARC, DMnd, pSPARC->DMVertices_dmcomm, 
                        ncol, 0.0, pSPARC->Xorb_kpt+spn_i*size_s+kpt*size_k, 
                        dpsi_full, 2, &kpt_vec, pSPARC->dmcomm);
