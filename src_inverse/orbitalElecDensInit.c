...
void Init_electronDensity(SPARC_OBJ *pSPARC) {
    ...
    if (pSPARC->dmcomm_phi != MPI_COMM_NULL) {
        int  i, DMnd;
        DMnd = pSPARC->Nd_d * (pSPARC->Nspden/2*2+1);
...
void Init_orbital(SPARC_OBJ *pSPARC)
{
    ...
    if((pSPARC->elecgs_Count) == 0){
        if (pSPARC->isGammaPoint){
            ...
            pSPARC->Yorb = (double *)malloc( max(size_k,DMnd) * sizeof(double) ); 
            ...
            //       in some cases, Nband_bandcomm = 0, so we need to at
            //       least assign memory for 1 band for 1 kpoint to Yorb
            pSPARC->Yorb_kpt = (double _Complex *) malloc( max(size_k,DMnd) * sizeof(double _Complex) ); 
            //pSPARC->Yorb = (double *)malloc( len_tot * sizeof(double) ); 
