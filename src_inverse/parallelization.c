...
void Setup_Comms(SPARC_OBJ *pSPARC) {
    ...
    // create a communicator that includes all active processes in kptcomm
    int proc_active = (pSPARC->bandcomm_index < 0 || pSPARC->dmcomm == MPI_COMM_NULL) ? 0 : 1;
    // kptcomm_active split by whether it's active
    MPI_Comm_split(pSPARC->kptcomm, proc_active, rank_kptcomm, &pSPARC->kptcomm_active);
    ...
    if (pSPARC->isGammaPoint){
        pSPARC->Hp = (double *)calloc(pSPARC->nr_Hp_BLCYC * pSPARC->nc_Hp_BLCYC , sizeof(double));
        pSPARC->Mp = (double *)calloc(pSPARC->nr_Mp_BLCYC * pSPARC->nc_Mp_BLCYC , sizeof(double));
        pSPARC->Q  = (double *)calloc(pSPARC->nr_Q_BLCYC * pSPARC->nc_Q_BLCYC , sizeof(double));
    } else{
        pSPARC->Hp_kpt = (double _Complex *) calloc(pSPARC->nr_Hp_BLCYC * pSPARC->nc_Hp_BLCYC, sizeof(double _Complex));
        pSPARC->Mp_kpt = (double _Complex *) calloc(pSPARC->nr_Mp_BLCYC * pSPARC->nc_Mp_BLCYC, sizeof(double _Complex));
        pSPARC->Q_kpt  = (double _Complex *) calloc(pSPARC->nr_Q_BLCYC * pSPARC->nc_Q_BLCYC, sizeof(double _Complex));
    }
    ...
    pSPARC->Veff_loc_kptcomm_topo = (double *)malloc( pSPARC->Nd_d_kptcomm * pSPARC->Nspden * sizeof(double) );
    ...
    if (pSPARC->dmcomm_phi != MPI_COMM_NULL) {
        ...
        pSPARC->electronDens_at = (double *)malloc( DMnd * ncopy * sizeof(double) );
        pSPARC->electronDens_core = (double *)calloc( DMnd * ncopy, sizeof(double) );
        ...
        pSPARC->electronDens = (double *)malloc( DMnd * ncopy * sizeof(double) );
        ...
        pSPARC->XCPotential = (double *)malloc( DMnd * pSPARC->Nspden * sizeof(double) );
        ...
        if(pSPARC->isgradient) {
            int ncopy = pSPARC->Nspden/2*2+1;
            pSPARC->Dxcdgrho = (double *)malloc( DMnd * ncopy * sizeof(double) );
        ...
        if (pSPARC->MixingVariable == 1) { // for potential mixing, the history is stored already
            ...
            pSPARC->Veff_loc_dmcomm_phi_in = (double *)malloc(DMnd * pSPARC->Nspden * sizeof(double));
 ...
 */
void SPARC_Dims_create(
    const int nproc, const int ndims, const int *gridsizes, 
    const int minsize_inpt, int *dims, int *ierr)
{
#define NPSTRIDE 5
#define NPSTRIDE3D 2
    *ierr = 0;
    int minsize = max(minsize_inpt,0); // minsize should be greater than or equal to 0
    if (ndims == 1) {
