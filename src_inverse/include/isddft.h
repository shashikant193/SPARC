...
/**
 * @brief This is a helper structure that can store data for functions.
 *
 */
...
typedef struct _SPARC_OBJ{
    ...
    /* INVERSE OFDFT PARAMETERS (total 14) */
    char fname_Vp_guess[L_STRING];
    char fname_rho_data[L_STRING];
    int INVERSE_FLAG;
    int Vxc_flag_inverse;
    int VTF_flag_inverse;
    int print_Vp_flag_inverse; 
    int read_Vp_guess_flag_inverse; // 0: zeros, 1: random, 2: from a file
    int BFGS_MAXITER_inverse;
    int BFGS_history_inverse;
    int MAXITER_KRYLOV_SCHUR_inverse;
    int MAXITER_CG_inverse;
    double TOL_CG_inverse;
    double BFGS_TOL_inverse;
    double KRYLOV_SCHUR_TOL_inverse;

    double *rho_data_inverse;
    double *phi_data_inverse;
    double *Vxc_data_inverse;
    double *VTF_data_inverse;
    double *Vp_inverse;
    double *dL_dVp;
    double *x_guess_inverse;
    double count_invert_derivative;
    int rank_last_proc_inverse;
    ...
    int Nspin;          // number of spin channel in a calculation. 
    ...
    /* Complementary Subspace (CS) */
    int CS_Flag; // Flag to turn on the CS method
    int CS_Nt;   // Number of top states used in the CS method
    int CS_npl;  // Number of polynomial degree used in 2nd level CheFSI
    int CS_ncheb; // Number of repeated chebyshev filtering steps in each 2nd level CheFSI
    double *tr_Hp_k; // trace of the projected Hamiltonian
    double *CS_Qt; // copy of top Nt subspace eigvectors
    int CS_nv_local; // number of local columns of subspace eigenvectors in band parallelization
    int CS_descQt[9]; // descriptor for Qt in band parallelization (BP)
    
    /* Discrete Discontinuous Basis Projection (DDBP) */
    int DDBP_Flag; 
    int DDBP_nALB_atom; // number of basis functions per atom
    int DDBP_Nex; // number of elements in x direction
    int DDBP_Ney; // number of elements in y direction
    int DDBP_Nez; // number of elements in z direction
    int DDBP_EBCx;    // element boundary conditions
    int DDBP_EBCy;    // element boundary conditions
    int DDBP_EBCz;    // element boundary conditions
    int DDBP_npelem;  // number of element groups (paral parameter)
    int DDBP_npbasis; // number of basis groups (paral parameter)
    double DDBP_buffer_x; // buffer length for extended element in x direction
    double DDBP_buffer_y; // buffer length for extended element in y direction
    double DDBP_buffer_z; // buffer length for extended element in z direction
    double DDBP_tol_updatebasis_first_EGS; // tol for updating DDBP basis for the very first electronic ground-state
    double DDBP_tol_updatebasis; // tol for updating DDBP basis for general EGS (other than the 1st MD/relax)
    DDBP_INFO *DDBP_info;
    ...
    double exx_frac;                // fraction of exact exchange
...
typedef struct _SPARC_INPUT_OBJ{ 
    ...
    int VTF_flag_inverse;
    int print_Vp_flag_inverse; 
    int read_Vp_guess_flag_inverse; // 0: random, 1: zeros, 2: from file
    int BFGS_MAXITER_inverse;
    int BFGS_history_inverse;
    int MAXITER_KRYLOV_SCHUR_inverse;
    int MAXITER_CG_inverse;
