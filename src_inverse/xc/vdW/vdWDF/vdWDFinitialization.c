...
void vdWDF_initial_read_kernel(SPARC_OBJ *pSPARC) {
    ...
    if (pSPARC->dmcomm_phi != MPI_COMM_NULL) {
        ...
        for (direction = 0; direction < 3; direction++) {
            pSPARC->Drho[direction] = (double*)malloc(DMnd * pSPARC->Nspden * sizeof(double)); assert(pSPARC->Drho[direction] != NULL);
        }
        pSPARC->gradRhoLen = (double*)malloc(DMnd * pSPARC->Nspden * sizeof(double));
        pSPARC->vdWDFecLinear = (double*)malloc(DMnd * sizeof(double)); assert(pSPARC->vdWDFecLinear != NULL); // \epsilon_cl
        pSPARC->vdWDFVcLinear = (double*)malloc(DMnd * pSPARC->Nspden * sizeof(double)); assert(pSPARC->vdWDFVcLinear != NULL); // d(n\epsilon_cl)/dn in dmcomm_phi
        pSPARC->vdWDFq0 = (double*)malloc(sizeof(double)*DMnd);
        pSPARC->vdWDFdq0drho = (double*)malloc(DMnd * pSPARC->Nspden * sizeof(double)); assert(pSPARC->vdWDFdq0drho != NULL);
        pSPARC->vdWDFdq0dgradrho = (double*)malloc(DMnd * pSPARC->Nspden * sizeof(double)); assert(pSPARC->vdWDFdq0dgradrho != NULL);
        ...
        pSPARC->vdWDFpotential = (double*)malloc(DMnd * pSPARC->Nspden * sizeof(double)); assert(pSPARC->vdWDFpotential != NULL);
        // if (rank == size - 1) {
