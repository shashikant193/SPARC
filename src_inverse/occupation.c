...
double Calculate_occupation(SPARC_OBJ *pSPARC, double x1, double x2, double tol, int max_iter)
{
    ...
    if (pSPARC->SQ3Flag == 1) {
        Efermi = Calculate_FermiLevel(pSPARC, x1, x2, tol, max_iter, occ_constraint_SQ3);
    } else {
        if (pSPARC->ext_FPMD_Flag != 0) {
            Efermi = Calculate_FermiLevel(pSPARC, x1, x2, tol, max_iter, occ_constraint_extFPMD);
        } else {
    ...
    MPI_Bcast(&Efermi, 1, MPI_DOUBLE, 0, pSPARC->kptcomm);

    // find occupations
    if (pSPARC->isGammaPoint) { // for gamma-point systems
        for (spn_i = 0; spn_i < pSPARC->Nspin_spincomm; spn_i++) {
            for (n = 0; n < Ns; n++) {
                if (pSPARC->CS_Flag == 1 && n < Ns-Nt) {
                    g_nk = 1.0;
                } else {
 ...
 *          of the occupations in the Brillouin zone, assuming the first Ns-Nt
 *          occupations are fully occupied when CSFlag is on.
 */
double occ_constraint(SPARC_OBJ *pSPARC, double lambda_f)
{
    ...
    double g = 0.0, g_nk = 0.0, Ne = pSPARC->NegCharge; 
    if (pSPARC->isGammaPoint) { // for gamma-point systems
        for (spn_i = 0; spn_i < pSPARC->Nspin_spincomm; spn_i++) {
            for (n = 0; n < Ns; n++) {
                if (CS_Flag == 1 && n < Ns-Nt) {
                    g_nk = 1.0;
                } else {
                    g_nk = smearing_function(
        ...
        for (spn_i = 0; spn_i < pSPARC->Nspin_spincomm; spn_i++) {
            for (k = 0; k < Nk; k++) {
                for (n = 0; n < Ns; n++) {
                    ...
                    g += (2.0/pSPARC->Nspin/pSPARC->Nspinor) * pSPARC->kptWts_loc[k] * g_nk;
                }
            }
        }    
        g *= 1.0 / pSPARC->Nkpts; // find average
        if (pSPARC->npspin != 1) { // sum over processes with the same rank in spincomm to find g
            // This barrier could fix a rare parallelization bug but slow down the code. 
            // TODO: gather eigenvalues before finding fermi-level 
            // MPI_Barrier(pSPARC->spin_bridge_comm);
            MPI_Allreduce(MPI_IN_PLACE, &g, 1, MPI_DOUBLE, MPI_SUM, pSPARC->spin_bridge_comm);
        }
