...
void init_guess_Lanczos(
    ...
    if (RandFlag == 1) {
        ...
        // use simple for loop
        // for (kk = 0; kk < DMnz; kk++) {
        //    k_global = kk + DMVert[4]; // global coord
        //    vz = c1 * cos(kz * k_global * dz) + c2 * sin(kz * k_global * dz);
        //    for (jj = 0; jj < DMny; jj++) {
        //        j_global = jj + DMVert[2]; // global coord
        //        vy = c1 * cos(ky * j_global * dy) + c2 * sin(ky * j_global * dy);
        //        for (ii = 0; ii < DMnx; ii++) {
        //            i_global = ii + DMVert[0]; // global coord
        //            vx = c1 * cos(kx * i_global * dx) + c2 * sin(kx * i_global * dx);
...
void eigSolve_CheFSI(int rank, SPARC_OBJ *pSPARC, int SCFcount, double error) {
    ...
    if (SCFcount == 0) {
        ...
        meshes[0] = pSPARC->delta_x;
        meshes[1] = pSPARC->delta_y;
        meshes[2] = pSPARC->delta_z;
        int gridsizes[3];
    ...
    if (pSPARC->DDBP_Flag == 1) {
        for (spn_i = 0; spn_i < pSPARC->Nspin_spincomm; spn_i++) {
            Calculate_DDBP_Basis_DDBP_Hamiltonian(pSPARC, count, 0, spn_i);
        }
    }
    ...
    while(count < pSPARC->rhoTrigger + SCFcount*pSPARC->Ncheb){
        ...
        for (spn_i = 0; spn_i < pSPARC->Nspin_spincomm; spn_i++) {
            if (pSPARC->DDBP_Flag == 1) {
                CheFSI_DDBP(pSPARC, lambda_cutoff, x0, count, 0, spn_i);
            } else {
        ...
        if (pSPARC->ext_FPMD_Flag != 0) {
            int Nc_e = pSPARC->Nstates - 1; // do these in initialization
            int Nc_s = max(0, Nc_e - 20); // TODO: hard-coded, make this an input!
            double U0 = calculate_effective_energy_shift_U0(pSPARC, Nc_s, Nc_e);
            pSPARC->ext_FPMD_U0 = U0;
        }
        ...
        if (pSPARC->SQ3Flag == 1){
            if (pSPARC->Dscomm != MPI_COMM_NULL)
                pSPARC->Efermi = Calculate_FermiLevel(pSPARC, eigmin_g-1.0, eigmax_g+1.0, 1e-12, 100, occ_constraint_SQ3); 
    ...
    if (pSPARC->DDBP_Flag == 1) {
        ...
        int kpt = 0; // for gamma point
        for (spn_i = 0; spn_i < pSPARC->Nspin_spincomm; spn_i++) {
            double **psi_ks = DDBP_info->psi[spn_i][kpt];
            // TODO: transfer psi to band+domain after SCF is converged!
            DDBP_ARRAY *X_ks = &DDBP_info->xorb[spn_i][kpt];
            recover_orbitals_on_grid(
                DDBP_info, X_ks, DDBP_info->desc_xorb, psi_ks,
                DDBP_info->desc_psi, nkpt, kpt, spn_i);
        }
        t2 = MPI_Wtime();
        #ifdef DEBUG
        if(!rank) printf("Total time for calculating psi on the grid (DDBP): %.3f ms\n", (t2-t1)*1e3);
        #endif
    }
...
void CheFSI(SPARC_OBJ *pSPARC, double lambda_cutoff, double *x0, int count, int k, int spn_i)
{
    ...
    if (pSPARC->SQ3Flag == 1) {
        ...
        int repeat_chefsi = 1;
        if (count == 0) {
            // if (!rank) printf(RED "Using eig for solving the partial eigenproblem!\n" RESET);
            // Solve_standard_EigenProblem(pSPARC, k, spn_i);
            // Solve_partial_standard_EigenProblem(pSPARC, pSPARC->CS_Nt, k, spn_i);
            // TODO: distribute Q to BP as next guess for dense CheFSI   
            repeat_chefsi = pSPARC->CS_ncheb;
            Solve_partial_standard_EigenProblem_CheFSI(
                pSPARC, lambda_cutoff, pSPARC->CS_Nt, repeat_chefsi, k, count, spn_i); 
        } else {
            repeat_chefsi = pSPARC->CS_ncheb;
            Solve_partial_standard_EigenProblem_CheFSI(
                pSPARC, lambda_cutoff, pSPARC->CS_Nt, repeat_chefsi, k, count, spn_i);
    ...
    // ** subspace rotation ** //
    if (pSPARC->CS_Flag == 1) {
        if (!rank) printf("Start partial rotation ...\n");

        #if defined(USE_DP_SUBEIG) && defined(USE_MKL)
        t1 = MPI_Wtime();
        int ONE = 1;
        int Nd_blacscomm = pSPARC->is_domain_uniform ? pSPARC->Nd : pSPARC->Nd_d_dmcomm;
        // distribute orbitals into block cyclic format 
        pdgemr2d_(&Nd_blacscomm, &pSPARC->Nstates, pSPARC->Xorb + spn_i*size_s, &ONE, &ONE, pSPARC->desc_orbitals,
        ...
        // #ifdef USE_DP_SUBEIG
        // if (!rank) printf("DP part to be implemented, exiting ...\n");
        // exit(1);
        // DP_Subspace_partial_Rotation(pSPARC, pSPARC->Xorb + spn_i*size_s); // not fully correct
        // #else
...
void Solve_partial_standard_EigenProblem(SPARC_OBJ *pSPARC, int Nt, int k, int spn_i) 
{
    ...
    if (pSPARC->useLAPACK == 1) {
        ...
        int ZERO = 0, ONE = 1, il = Ns-Nt+1, iu = Ns, lwork, *iwork, liwork, *ifail, 
...
void DP_Project_Hamiltonian_std(SPARC_OBJ *pSPARC, int *DMVertices, double *Y, int spn_i)
{
    ...
    Hamiltonian_vectors_mult(
        ...
        Veff_loc_sg, pSPARC->Atom_Influence_nloc, 
        pSPARC->nlocProj, pSPARC->Nband_bandcomm, 
        0.0, pSPARC->Xorb + spn_i*size_s, Y, spn_i, pSPARC->dmcomm
    );
    et = MPI_Wtime();
    #ifdef DEBUG
    ...
    { // Enclose in brackets original CPU if(useLAPACK...) block and put it at the bottom
        ...
        if (pSPARC->useLAPACK == 1)
        {
            ...
            if (pSPARC->StandardEigenFlag == 0) {
                ...
                if (rank_kpt == 0) printf("DP_Solve_Generalized_EigenProblem rank 0 used %.3lf ms, LAPACKE_dsyevd used %.3lf ms\n", 1000.0 * (et1 - st), 1000.0 * (et0 - st));
            }
            #endif
        } else {
            #if defined(USE_MKL) || defined(USE_SCALAPACK)
                         ...
                         double *Hp, double *Mp, int k, int spn_i, MPI_Comm comm) 
{
    ...
    int rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    if (rank == 0) printf("[FATAL] Subspace projection are using ScaLAPACK routines but ScaLAPACK is not compiled\n");
    if (rank == 0) printf("\nPlease turn on USE_MKL or USE_SCALAPACK! Or turn on USE_DP_SUBEIG to perform projection without ScaLAPACK.\n");
    exit(255);
#endif // #if defined(USE_MKL) || defined(USE_SCALAPACK)
}
    ...
    double *Psi_rot, int Nt, int k, int spn_i)
{
    ...
    t1 = MPI_Wtime();
    // perform matrix multiplication Psi * Q using ScaLAPACK routines
    pdgemm_("N", "N", &Nd_blacscomm, &Nt, &Ns, &alpha, 
    ...
    // distribute rotated orbitals from block cyclic format back into 
    // original format (band + domain)
    pdgemr2d_(&Nd_blacscomm, &Nt, PsiQ, &ONE, &il, 
...
int Spectrum2ChebDegree(double sigma)
{
    ...
    const double lambda_1D_0 = 6.8761754299116333;
    const double lambda_3D_0 = 3 * lambda_1D_0; // 3D
    // given spectrum width, find the effective mesh
    double h_eff = sqrt(lambda_3D_0 / sigma);
    return Mesh2ChebDegree(h_eff);
}
