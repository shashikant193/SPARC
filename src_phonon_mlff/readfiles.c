...
void read_input(SPARC_INPUT_OBJ *pSPARC_Input, SPARC_OBJ *pSPARC) {
    ...
    while (!feof(input_fp)) {
        ...
        if (strcmpi(str,"NP_SPIN_PARAL:") == 0) {
           ...
           fscanf(input_fp, "%*[^\n]\n");
            ...
            fscanf(input_fp, "%*[^\n]\n");
        } else if(strcmpi(str,"MLFF_PHONON_FLAG:") == 0) {
            // pSPARC->mlff_flag == 1 means on-the-fly MD from scratch, pSPARC->mlff_flag == 21 means only prediction using a known model, pSPARC->mlff_flag == 22 on-the-fly MD starting from a known model
            fscanf(input_fp,"%d",&pSPARC_Input->mlff_phonon_flag);
            fscanf(input_fp, "%*[^\n]\n");
            ...
            fscanf(input_fp, "%*[^\n]\n");
         } else if (strcmpi(str,"MLFF_MODEL_FOLDER:") == 0) {
        ...
        /* SQ input options */
            ...
            fscanf(input_fp, "%*[^\n]\n");
        } else if (strcmpi(str,"SQ_GAUSS_HYBRID_MEM:") == 0) {    
            fscanf(input_fp,"%s",temp);
            if (strcmpi(temp,"LOW") == 0 || strcmpi(temp,"low") == 0) {
                pSPARC_Input->SQ_gauss_hybrid_mem = 0;
            } else if (strcmpi(temp,"HIGH") == 0 || strcmpi(temp,"high") == 0) {
                pSPARC_Input->SQ_gauss_hybrid_mem = 1;
            } else {
        ...
        /* OFDFT input options */
            ...
            fscanf(input_fp, "%*[^\n]\n");
        } else if(strcmpi(str,"SP_FLAG:") == 0) {
            fscanf(input_fp,"%d",&pSPARC_Input->spDFT_Flag);
            fscanf(input_fp, "%*[^\n]\n");
        } else if (strcmpi(str,"SP_SMEARING:") == 0) {
            fscanf(input_fp,"%lf",&pSPARC_Input->spDFT_tau_s);
            fscanf(input_fp, "%*[^\n]\n");
        } else if (strcmpi(str,"SP_TOL_OCC:") == 0) {
            fscanf(input_fp,"%lf",&pSPARC_Input->spDFT_tol_occ);
            fscanf(input_fp, "%*[^\n]\n");
        } else if (strcmpi(str,"PRINT_SPDFT:") == 0) {
            fscanf(input_fp,"%d",&pSPARC_Input->PrintspDFTFlag);
            fscanf(input_fp, "%*[^\n]\n");
        } else if (strcmpi(str,"SP_ISESPLIT_CONST:") == 0) {
            fscanf(input_fp,"%d",&pSPARC_Input->spDFT_isesplit_const);
            fscanf(input_fp, "%*[^\n]\n");
        }
#ifdef USE_SOCKET
    ...
    //if (pSPARC_Input->mlff_flag > 0 && ((strcmpi(pSPARC_Input->MDMeth,"NPT_NP")==0) || (strcmpi(pSPARC_Input->MDMeth,"NPT_NH")==0))) {
    //    printf("\nMLFF not supported for NPT currently!\n");
    //    exit(EXIT_FAILURE);
    //}
