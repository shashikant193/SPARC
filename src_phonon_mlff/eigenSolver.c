...
void eigSolve_CheFSI(int rank, SPARC_OBJ *pSPARC, int SCFcount, double error) {
    ...
    while(count < pSPARC->rhoTrigger + SCFcount*pSPARC->Nchefsi){
		...
		if (pSPARC->spDFT_Flag == 1) {
			spDFT_eigen_minmax(pSPARC, &eigmin_g, &eigmax_g);
		}
...
void CheFSI(SPARC_OBJ *pSPARC, double lambda_cutoff, double *x0, int count, int k, int spn_i)
{
    ...
    if (pSPARC->SQ3Flag == 1) {
        ...
        Solve_partial_standard_EigenProblem_CheFSI(
            pSPARC, lambda_cutoff, pSPARC->CS_Nt, pSPARC->CS_ncheb, k, count, spn_i);
    } else {
    ...
    // ** subspace rotation ** //
    if (pSPARC->SQ3Flag == 1) {
        // nothing to do here, but need to skip subspace rotation
    } else if (pSPARC->CS_Flag == 1) {
        Subspace_partial_Rotation_CS(pSPARC, k, spn_i);
    } else {
        #ifdef USE_DP_SUBEIG
