 ...
 * @param proc_active Flag to indicate whether the process is active or not.
 */
void DDBP_Element_Basis_Coeff_Multiply(
    const double *vk, int *descvk, const double *xk, int *descxk,
    double *yk, int *descyk, MPI_Comm rowcomm, int proc_active)
{
    if (rowcomm == MPI_COMM_NULL || proc_active == 0) return;
...
)
{
    ...
    restrict_to_subgrid(
        ...
        is, ie, js, je, ks, ke, ips, jps, kps, sizeof(double)
    );
}
