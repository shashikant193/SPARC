...
void create_DDBP_bandcomm(DDBP_INFO *DDBP_info, MPI_Comm comm) {
    ...
    if (DDBP_info->basiscomm_index < 0) DDBP_info->bandcomm_index = -1;
...
/**
 * @brief   Set up active sub-communicators basiscomm for distributing DDBP basis functions.
 *
 * @param comm       Communicator to be splitted.
 */
void create_active_subcomm(MPI_Comm comm, int is_active, MPI_Comm *active_comm) {
    int rank;
    MPI_Comm_rank(comm, &rank);
    MPI_Comm_split(comm, is_active, rank, active_comm);
}
...
void allocate_init_DDBP_variables(SPARC_OBJ *pSPARC) {
...
#if defined(USE_MKL) || defined(USE_SCALAPACK)
    // create a context corresponding to rowcomm (original grid)
    int nproc_rowcomm;
    MPI_Comm_size(DDBP_info->rowcomm, &nproc_rowcomm);
    int bhandle = Csys2blacs_handle(DDBP_info->rowcomm); // create a context out of rowcomm
    int ictxt = bhandle;
    // for original blacscomm context
    int dims[2];
    dims[0] = 1; dims[1] = nproc_rowcomm;
    Cblacs_gridinit(&ictxt, "Row", dims[0], dims[1]);
    ...
    SPARC_Dims_create(min(nproc_rowcomm,np_best), 2, gridsizes, 256, dims, &ierr);
    ierr = 1;
    int ishift = 8;
    while (ierr && ishift) {
        SPARC_Dims_create(min(nproc_rowcomm,np_best), 2, gridsizes, 1<<ishift, dims, &ierr);
        ishift--;
    }
    ...
    bhandle = Csys2blacs_handle(DDBP_info->rowcomm);
...
void Setup_Comms_DDBP(SPARC_OBJ *pSPARC) {
    ...
    // create a rowcomm that includes all active basiscomms (basiscomm_index >= 0)
    DDBP_info->is_active_rowcomm = (DDBP_info->basiscomm_index) >= 0;
    create_active_subcomm(DDBP_info->elemcomm, DDBP_info->is_active_rowcomm, &DDBP_info->rowcomm);
}
