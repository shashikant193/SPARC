...
void free_DDBP_info(SPARC_OBJ *pSPARC) {
    ...
    // TODO: free any sub items of DDBP_info here ...
        ...
        if(DDBP_info->rowcomm != MPI_COMM_NULL)
            MPI_Comm_free(&DDBP_info->rowcomm);
    ...
    int kpt, int spn_i)
{
    ...
    for (int rpt = 0; rpt < ChebRepeat; rpt++) {
        ...
        Project_Hamiltonian_DDBP(
            ...
            kpt, spn_i, DDBP_info->rowcomm, DDBP_info->bandcomm
        ...
        int proc_active = (int) (DDBP_info->elemcomm_index >= 0 && DDBP_info->is_active_rowcomm);
        Solve_Subspace_EigenProblem_DDBP(
            ...
            DDBP_info->rowcomm, pSPARC->eig_paral_blksz, pSPARC->eig_paral_maxnp, proc_active);
        t2 = MPI_Wtime();
        ...
        Subspace_Rotation_DDBP(
            ...
            X_ks, DDBP_info->desc_xorb, DDBP_info->rowcomm, proc_active
        );
        t2 = MPI_Wtime();
        #ifdef DEBUG
