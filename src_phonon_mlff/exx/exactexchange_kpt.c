    ...
    double _Complex *psi_outer, double _Complex *psi, double *occ_outer, int spn_i, double _Complex *Xi_kpt)
{
    ...
    for (kpt_k = 0; kpt_k < pSPARC->Nkpts_kptcomm; kpt_k ++) {
        ...
        for (loop = 0; loop < NL; loop ++) {
            ...
            t1 = MPI_Wtime();
            poissonSolve_kpt(pSPARC, rhs, count-base, DMnd, dims, Vi, kpt_k_list, kpt_q_list, pSPARC->dmcomm);
            t2 = MPI_Wtime();
...
void exact_exchange_potential_kpt(SPARC_OBJ *pSPARC, 
        double _Complex *X, int ncol, int DMnd, double _Complex *Vexx, int spin, int kpt, MPI_Comm comm) 
{        
    ...
    if (pSPARC->ACEFlag == 0) {
        ...
        evaluate_exact_exchange_potential_kpt(pSPARC, X, ncol, DMnd, dims, occ_outer, psi_outer, Vexx, kpt, comm);

    } else {
        Xi = (Lanczos_flag == 0) ? pSPARC->Xi_kpt + spin * xi_shift : pSPARC->Xi_kptcomm_topo_kpt + spin * xi_shift;
        evaluate_exact_exchange_potential_ACE_kpt(pSPARC, X, ncol, DMnd, Xi, Vexx, spin, kpt, comm);
    }
...
void evaluate_exact_exchange_potential_kpt(SPARC_OBJ *pSPARC, double _Complex *X, 
        ...
        double _Complex *psi_outer, double _Complex *Vexx, int kpt_k, MPI_Comm comm) 
{
    ...
    for (loop = 0; loop < NL; loop ++) {
        ...
        poissonSolve_kpt(pSPARC, rhs, count-base, DMnd, dims, Vi, kpt_k_list, kpt_q_list, comm);
        t2 = MPI_Wtime();
        pSPARC->FFTtime += (t2-t1);
        ...
        for (count = batch_num_rhs*loop; count < min(batch_num_rhs*(loop+1),num_rhs); count++) {
            ...
            if (pSPARC->kpthf_pn[l] == 1) {
                for (k = 0; k < DMnd; k++) 
                    Vexx[k + i*DMnd] -= occ * pSPARC->kptWts_hf * psi_outer[k + j*DMnd + ll_red*DMndNs] * Vi[k + (count-base)*DMnd] / pSPARC->dV;
            } else {
                for (k = 0; k < DMnd; k++) 
                    Vexx[k + i*DMnd] -= occ * pSPARC->kptWts_hf * conj(psi_outer[k + j*DMnd + ll_red*DMndNs]) * Vi[k + (count-base)*DMnd] / pSPARC->dV;
            }
        }
    }
...
void evaluate_exact_exchange_potential_ACE_kpt(SPARC_OBJ *pSPARC, 
        ...
        double _Complex *Vexx, int spin, int kpt, MPI_Comm comm) 
{
    ...
    cblas_zgemm( CblasColMajor, CblasNoTrans, CblasNoTrans, DMnd, ncol, Ns_occ,
                &alpha, Xi + kpt*DMndNs_occ, DMnd, Xi_times_psi, Ns_occ, &beta, Vexx, DMnd);
 ...
 */
void poissonSolve_kpt(SPARC_OBJ *pSPARC, double _Complex *rhs, int ncol, int DMnd, 
                int *dims, double _Complex *Vi, int *kpt_k_list, int *kpt_q_list, MPI_Comm comm) 
{
    ...
    if (pSPARC->EXXMeth_Flag == 0) {                                                // Solve in Fourier Space
        pois_fft_kpt(pSPARC, rhs_loc_order, ncolp, Vi_loc, kpt_k_list + kq_shift, kpt_q_list + kq_shift);
    } else {
 ...
 */
void pois_fft_kpt(SPARC_OBJ *pSPARC, double _Complex *rhs_loc_order, int ncolp, 
                double _Complex *Vi_loc, int *kpt_k_list, int *kpt_q_list) 
{
#define Kptshift_map(i,j) Kptshift_map[i+j*pSPARC->Nkpts_sym]
    ...
    for (j = 0; j < ncolp; j++) {
        ...
        if (!l) {
            alpha = pSPARC->pois_FFT_const + Nd * (pSPARC->Nkpts_shift - 1);
        } else {
            alpha = pSPARC->pois_FFT_const + Nd * (l - 1);
        }
        for (i = 0; i < Nd; i++) {
...
#define alpha(i,j,k,l) alpha[(l)*Nd+(k)*Nxy+(j)*Nx+(i)]
            ...
            for (k = 0; k < Nz; k++) {
                for (j = 0; j < Ny; j++) {
                    for (i = 0; i < Nx; i++) {
                        ...
                        G2 = G[0] * g[0] + G[1] * g[1] + G[2] * g[2];
                        if (fabs(G2) > 1e-4) {                                              // 1e-4 is the tolerance from ABINIT
                            alpha(i,j,k,l) = 4*M_PI*(1-exp(-G2/4/omega2))/G2;
                        } else {
