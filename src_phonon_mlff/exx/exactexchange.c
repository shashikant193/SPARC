...
void Exact_Exchange_loop(SPARC_OBJ *pSPARC) {
    ...
    pSPARC->Exxcount = 0;
    pSPARC->Exxtime = pSPARC->FFTtime = 0.0;
    ...
    while (err_Exx > pSPARC->TOL_FOCK && count_xx < pSPARC->MAXIT_FOCK) {
    #ifdef DEBUG
    if(!rank) 
        ...
        if (count_xx == 0 && pSPARC->PrintEnergyDensFlag == 1) {
            t1 = MPI_Wtime();
            computeExactExchangeEnergyDensity(pSPARC, pSPARC->ExxRho_approx);
            t2 = MPI_Wtime();
        #ifdef DEBUG
        if (rank == 0) printf("Time for calculating approximate exact exchange energy density: %.3f ms\n", (t2-t1)*1e3);
        #endif
        }
    ...
    #ifdef DEBUG
    if(!rank) 
    printf("\nEvaluating %d times exact exchange took %.3f s, in which FFT took %.3f s\n\n",
                pSPARC->Exxcount, pSPARC->Exxtime, pSPARC->FFTtime);
    #endif  
...
void ACE_operator(SPARC_OBJ *pSPARC, double *psi_outer, double *occ_outer, int spn_i, double *Xi) {
    ...
    /* EXXACEMem_batch could be any positive integer to define the maximum number of 
       Poisson's equations to be solved at one time. The smaller the EXXACEMem_batch, 
    ...
    batch_num_rhs = pSPARC->EXXACEMem_batch == 0 ? 
                        num_rhs : pSPARC->EXXACEMem_batch * nproc_dmcomm;
    ...
    for (loop = 0; loop < NL; loop ++) {
        ...
        t1 = MPI_Wtime();
        poissonSolve(pSPARC, rhs, count-base, DMnd, dims, Vi, pSPARC->dmcomm);
        t2 = MPI_Wtime();
        pSPARC->FFTtime += (t2-t1);
 ...
 */
void exact_exchange_potential(SPARC_OBJ *pSPARC, double *X, int ncol, int DMnd, double *Vexx, int spin, MPI_Comm comm) {        
    ...
    if (pSPARC->ACEFlag == 0) {
        ...
        evaluate_exact_exchange_potential(pSPARC, X, ncol, DMnd, dims, occ, psi, Vexx, comm);

    } else {
        Xi = (Lanczos_flag == 0) ? pSPARC->Xi + spin * xi_shift : pSPARC->Xi_kptcomm_topo + spin * xi_shift;
        evaluate_exact_exchange_potential_ACE(pSPARC, X, ncol, DMnd, Xi, Vexx, spin, comm);
    }
 ...
 * @param Vexx            Result of fock operator times X 
...
void evaluate_exact_exchange_potential(SPARC_OBJ *pSPARC, double *X, int ncol, int DMnd, int *dims, 
                                    double *occ_outer, double *psi_outer, double *Vexx, MPI_Comm comm) 
{
    int i, j, k, rank, Ns, num_rhs, *rhs_list_i, *rhs_list_j;
    int size, batch_num_rhs, NL, base, loop;
    double occ, *rhs, *Vi, t1, t2;
    ...
    batch_num_rhs = pSPARC->EXXACEMem_batch == 0 ? 
                        num_rhs : pSPARC->EXXACEMem_batch * size;
    NL = (num_rhs - 1) / batch_num_rhs + 1;                                                // number of loops required                        
    rhs = (double *)malloc(sizeof(double) * DMnd * batch_num_rhs);                         // right hand sides of Poisson's equation
    Vi = (double *)malloc(sizeof(double) * DMnd * batch_num_rhs);                          // the solution for each rhs
    assert(rhs != NULL && Vi != NULL);
    ...
    for (loop = 0; loop < NL; loop ++) {
        ...
        poissonSolve(pSPARC, rhs, count-base, DMnd, dims, Vi, comm);
        t2 = MPI_Wtime();
        pSPARC->FFTtime += (t2-t1);
        ...
        for (count = batch_num_rhs*loop; count < min(batch_num_rhs*(loop+1),num_rhs); count++) {
            ...
            occ = occ_outer[j];
            for (k = 0; k < DMnd; k++) {
                Vexx[k + i*DMnd] -= occ * psi_outer[k + j*DMnd] * Vi[k + (count-base)*DMnd] / pSPARC->dV;
            }
        }
    }
...
void evaluate_exact_exchange_potential_ACE(SPARC_OBJ *pSPARC, 
    double *X, int ncol, int DMnd, double *Xi, double *Vexx, int spin, MPI_Comm comm) 
{
    ...
    if (size > 1 && !pSPARC->is_domain_uniform) {
        // sum over all processors in dmcomm
    ...
    cblas_dgemm( CblasColMajor, CblasNoTrans, CblasNoTrans, DMnd, ncol, Ns_occ,
                -1.0, Xi, DMnd, Xi_times_psi, Ns_occ, 0.0, Vexx, DMnd);
 ...
 */
void poissonSolve(SPARC_OBJ *pSPARC, double *rhs, int ncol, int DMnd, 
                int *dims, double *Vi, MPI_Comm comm) 
{
    ...
    if (pSPARC->EXXMeth_Flag == 0) {                                                // Solve in Fourier Space
        pois_fft(pSPARC, rhs_loc_order, ncolp, Vi_loc);
    } else {
 ...
 */
void pois_fft(SPARC_OBJ *pSPARC, double *rhs_loc_order, int ncolp, double *Vi_loc) {
    if (ncolp == 0) return;
    ...
    for (j = 0; j < ncolp; j++) {
        for (i = 0; i < Ndc; i++) {
            rhs_bar[i + j*Ndc] = creal(rhs_bar[i + j*Ndc]) * alpha[i] 
                                + (cimag(rhs_bar[i + j*Ndc]) * alpha[i]) * I;
        }
    }
...
#define alpha(i,j,k) alpha[(k)*Nxy+(j)*Nxh+(i)]
    ...
    double L1, L2, L3, V, R_c, *alpha, G[3], g[3], G2, omega, omega2;
        ...
        for (k = 0; k < Nz; k++) {
            for (j = 0; j < Ny; j++) {
                for (i = 0; i < Nxh; i++) {
                    ...
                    if (fabs(G2) > 1e-4) {
                        alpha(i,j,k) = 4*M_PI*(1-cos(R_c*sqrt(G2)))/G2;
                    } else {
        ...
        for (k = 0; k < Nz; k++) {
            for (j = 0; j < Ny; j++) {
                for (i = 0; i < Nxh; i++) {
                    ...
                    if (fabs(G2) > 1e-4) {
                        if (omega > 0) 
                            alpha(i,j,k) = 4*M_PI/G2 * (1 - exp(-0.25/omega2*G2));
                        else
        ...
        for (k = 0; k < Nz; k++) {
            for (j = 0; j < Ny; j++) {
                for (i = 0; i < Nxh; i++) {
                    ...
                    G2 = G[0] * g[0] + G[1] * g[1] + G[2] * g[2];
                    if (fabs(G2) > 1e-4) {
                        alpha(i,j,k) = 4*M_PI*(1-exp(-G2/4.0/omega2))/G2;
                    } else {
...
void auxiliary_constant(SPARC_OBJ *pSPARC) 
{
    ...
    int Nx, Ny, Nz, khf, i, j, k;
    double L1, L2, L3, V, alpha, scaled_intf, sumfGq;
    double tpiblx, tpibly, tpiblz, g[3], G[3], modGq, omega, omega2;
    ...
    sumfGq = 0;
    for (khf = 0; khf < pSPARC->Nkpts_hf; khf++) {
        for (k = 0; k < Nz; k++) {
            for (j = 0; j < Ny; j++) {
                for (i = 0; i < Nx; i++) {
                    // G = [(k1-1)*2*pi/L1, (k2-1)*2*pi/L2, (k3-1)*2*pi/L3];
                    ...
                    G[0] += pSPARC->k1_hf[khf];
                    G[1] += pSPARC->k2_hf[khf];
                    G[2] += pSPARC->k3_hf[khf];
                    matrixTimesVec_3d(pSPARC->lapcT, G, g);
...
void allocate_ACE(SPARC_OBJ *pSPARC) {
    ...
    DMnd = pSPARC->Nd_d_dmcomm;
    sum_temp = sum = 0;
    // make sure meaningless proc will always allocate empty Xi
    if (pSPARC->dmcomm == MPI_COMM_NULL) {
        Ns_occ_temp[0] = Ns_occ_temp[1] = 1;
        sum_temp = 2;
    }
