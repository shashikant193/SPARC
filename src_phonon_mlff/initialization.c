...
void Initialize(SPARC_OBJ *pSPARC, int argc, char *argv[]) {
	...
	if(pSPARC->spDFT_Flag == 1)
		spDFT_initialization(pSPARC, 0.0);
...
void set_defaults(SPARC_INPUT_OBJ *pSPARC_Input, SPARC_OBJ *pSPARC) {
    ...
    pSPARC_Input->SQ_gauss_hybrid_mem = 0;      // default not saving exact exchange potential
    ...
    pSPARC_Input->mlff_phonon_flag=0;
...
void SPARC_copy_input(SPARC_OBJ *pSPARC, SPARC_INPUT_OBJ *pSPARC_Input) {
    ...
    pSPARC->mlff_phonon_flag = pSPARC_Input->mlff_phonon_flag;
    ...
    pSPARC->SQ_gauss_hybrid_mem = pSPARC_Input->SQ_gauss_hybrid_mem;
	...
	pSPARC->spDFT_Flag = pSPARC_Input->spDFT_Flag;
	pSPARC->PrintspDFTFlag = pSPARC_Input->PrintspDFTFlag;
	pSPARC->spDFT_isesplit_const = pSPARC_Input->spDFT_isesplit_const;
	pSPARC->spDFT_tau_s = pSPARC_Input->spDFT_tau_s;
	pSPARC->spDFT_tol_occ = pSPARC_Input->spDFT_tol_occ;
    ...
    */
    ...
    if (rank == 0) {
		...
		snprintf(pSPARC->spDFTFilename,    L_STRING, "%s.spDFT",     pSPARC->filename_out);
    ...
    if (pSPARC->usefock == 1 && pSPARC->ExxAcc == 1 && !pSPARC->SQFlag){
        if (rank == 0)
            printf(RED "ERROR: To use hybrid functional with ACE method, please turn on MKL in makefile!\n"RESET);
        exit(EXIT_FAILURE);
    }
    ...
    if (pSPARC->DDBP_Flag == 1){
        if (rank == 0)
            printf(RED "ERROR: To use DDBP method, please turn on MKL in makefile!\n"
    ...
    if (pSPARC->usefock == 1) {
        if (pSPARC->SOC_Flag || pSPARC->OFDFTFlag) {
            if (!rank) 
                printf(RED "ERROR: Spin-orbit coupling, FDFT are not supported in this version of hybrid implementation.\n" RESET);
            exit(EXIT_FAILURE);
        }
        ...
        if (pSPARC->SQFlag == 1) {
            ...
            if (pSPARC->ExxMethod < 0) {
                ...
                if (pSPARC->ExxMethod == 1 && !(pSPARC->isGammaPoint == 1 && pSPARC->BC <= 2 && strcmpi(pSPARC->XC, "PBE0") == 0)) { // kronecker product Lap way
                    if (!rank) {
                        printf(YEL "WARNING: KRON method only supports orthogonal bulk and molecule at gamma point in this version of hybrid implementation. Changed to FFT method.\n" RESET);
                    }                    
                    pSPARC->ExxMethod = 0;
                } 
            }
    ...
    if (pSPARC->SQFlag == 1) {
        ...
        if (pSPARC->SQ3Flag || pSPARC->CS_Flag || pSPARC->SOC_Flag || pSPARC->ixc[2]) {
            if (!rank) 
                printf(RED "ERROR: SQ3, CS, spin-orbit coupling, and SCAN are not supported in this version of SQ implementation.\n" RESET);
            exit(EXIT_FAILURE);
        }
        if (pSPARC->SQ_rcut < 0) {
            if (!rank)
    ...
    if (pSPARC->SOC_Flag == 1) {
        if (pSPARC->SQFlag || pSPARC->usefock || pSPARC->ixc[2] || pSPARC->SQ3Flag || pSPARC->CS_Flag || pSPARC->OFDFTFlag) {
            if (rank == 0) 
    ...
    if (pSPARC->spin_typ == 2) {
        if (pSPARC->SQFlag || pSPARC->usefock || pSPARC->ixc[2] || pSPARC->CyclixFlag || pSPARC->SQ3Flag || pSPARC->CS_Flag || pSPARC->OFDFTFlag) {
            if (rank == 0) 
	...
	if (strcmpi(pSPARC->MDMeth,"NPT_NH") == 0 || strcmpi(pSPARC->MDMeth,"NPT_NP") == 0){
		if (rank == 0) printf("[WARNING] Currently, NPT simulations are done at constant number of finite-difference grid points. Grid-spacing can considerable change for large changes in cell dimensions\n");
	}
    ...
    if (strcmpi(pSPARC->MDMeth,"NPT_NP") == 0) {
        ...
        if (pSPARC->CyclixFlag) { // cyclix coordinate, only z direction can be rescaled
            if (!((pSPARC->NPTscaleVecs[0] == 0) && (pSPARC->NPTscaleVecs[1] == 0) && (pSPARC->NPTscaleVecs[2] == 1))) {
                if (!rank) {
                    printf("\nNPT_NP only supports expansion on z (axial) direction for cyclix coordinate. Please set NPT_SCALE_VECS: 3 \n");
                    printf("then set NPT_SCALE_CONSTRAINTS: none \n");
                }
                exit(EXIT_FAILURE);
            }
        }
        if (pSPARC->NPTconstraintFlag == 1) { // check conflict between NPT_SCALE_CONSTRAINTS and NPT_SCALE_VECS
	...
	/*if(pSPARC->mlff_flag > 0 && pSPARC->cell_typ > 0) {
		if (!rank) {
        	printf("\nCurrently, non-orthogonal cells are not supported with MLFF!\n");
        }
        exit(EXIT_FAILURE);
	}*/

	double celllen_min = min(min(pSPARC->range_x,pSPARC->range_y),pSPARC->range_z);
	
	if(pSPARC->mlff_flag > 0 && pSPARC->rcut_SOAP >= celllen_min) {
		if (!rank) {
        	printf("\nWarning: Self-images coming in the MLFF-Rcut will shift the energy, not considered currently (reduce Rcut )!\n");
        }
	}
        ...
        for (nk2 = nk2_s; nk2 < nk2_e; nk2++) {
            for (nk3 = nk3_s; nk3 < nk3_e; nk3++) {
                ...
                if (pSPARC->CyclixFlag) {
                    ...
                    k3_red = k3_red * 2.0 * M_PI / Lz;
                    k3_red += pSPARC->twist *nk2;
...
void write_output_init(SPARC_OBJ *pSPARC) {
    ...
    fprintf(output_fp,"***************************************************************************\n");
    fprintf(output_fp,"*                       SPARC (version June 16, 2024)                      *\n");
    ...
    if (pSPARC->SQFlag == 1) {
        ...
        if (pSPARC->usefock) {
            if (pSPARC->SQ_gauss_hybrid_mem == 1) {
                fprintf(output_fp,"SQ_GAUSS_HYBRID_MEM: HIGH\n");
            } else {
                fprintf(output_fp,"SQ_GAUSS_HYBRID_MEM: LOW\n");
            }
        }
	...
	if (pSPARC->mlff_flag>0){
        ...
        fprintf(output_fp, "MLFF_PHONON_FLAG: %d\n", pSPARC->mlff_phonon_flag);
    }

	if(pSPARC->spDFT_Flag == 1){
		fprintf(output_fp,"SP_FLAG: %d\n", pSPARC->spDFT_Flag);
		fprintf(output_fp,"PRINT_SPDFT: %d\n", pSPARC->PrintspDFTFlag);
		fprintf(output_fp,"SP_ISESPLIT_CONST: %d\n", pSPARC->spDFT_isesplit_const);
		fprintf(output_fp,"SP_SMEARING: %.15g\n", pSPARC->spDFT_tau_s);
		fprintf(output_fp,"SP_TOL_OCC: %.15g\n", pSPARC->spDFT_tol_occ);
	}
...
void SPARC_Input_MPI_create(MPI_Datatype *pSPARC_INPUT_MPI) {
    ...
    MPI_Datatype SPARC_types[N_MEMBR] = {MPI_INT, MPI_INT, MPI_INT,                       /* int array */
                                         ...
                                         MPI_INT, MPI_INT, MPI_INT, MPI_INT, MPI_INT, 
                                         MPI_INT, MPI_INT, MPI_INT, MPI_INT, MPI_INT,
                                         MPI_INT, MPI_INT, MPI_INT, MPI_INT, MPI_INT, /* int */
                                         MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE,
                                         MPI_DOUBLE, MPI_DOUBLE, /* double array */
                                         MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE,
                                         MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE,
                                         MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE,
                                         MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE,
                                         ...
                                         MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE,
                                         MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE, 
                                         MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE,  /*double */
    ...
    MPI_Get_address(&sparc_input_tmp.SQ_gauss_hybrid_mem, addr + i++);
    ...
    MPI_Get_address(&sparc_input_tmp.mlff_phonon_flag, addr + i++);
	...
	MPI_Get_address(&sparc_input_tmp.spDFT_Flag, addr + i++);
	MPI_Get_address(&sparc_input_tmp.PrintspDFTFlag, addr + i++);
	MPI_Get_address(&sparc_input_tmp.spDFT_isesplit_const, addr + i++);
    ...
    MPI_Get_address(&sparc_input_tmp.spDFT_tau_s, addr + i++);
	MPI_Get_address(&sparc_input_tmp.spDFT_tol_occ, addr + i++);
