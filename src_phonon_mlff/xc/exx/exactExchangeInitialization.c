    ...
    if (pSPARC->BC == 2) {
        ...
        if (pSPARC->ExxDivFlag == 0) {        
            for (int i = 0; i < pSPARC->Nd; i++) {
                ...
                if (fabs(G2) > 1e-4) {
                    pSPARC->pois_const[i] = 4*M_PI*(1-cos(x))/G2;
                    if (pSPARC->Calc_stress == 1) {
                        double G4 = G2*G2;
                        pSPARC->pois_const_stress[i] = 4*M_PI*( 1-cos(x)- x/2*sin(x) )/G4;
                        pSPARC->pois_const_stress2[i] = 4*M_PI*( x/2*sin(x) )/G2/3;
                    }
                } else {
                    pSPARC->pois_const[i] = 2*M_PI*(R_c * R_c);
                    if (pSPARC->Calc_stress == 1) {
                        pSPARC->pois_const_stress[i] = 4*M_PI*pow(R_c,4)/24;
                        pSPARC->pois_const_stress2[i] = 4*M_PI*( R_c*R_c/2 )/3;
                    }
                }
            }
        // auxiliary function
            ...
            for (int i = 0; i < pSPARC->Nd; i++) {
                ...
                if (fabs(G2) > 1e-4) {
                    ...
                    pSPARC->pois_const[i] = 4*M_PI*pSPARC->const_aux;
                    if (pSPARC->Calc_stress == 1) {
...
#define alpha(i,j,k) pSPARC->pois_const[(k)*Nxy+(j)*Nxh+(i)]
#define alpha1(i,j,k) pSPARC->pois_const_stress[(k)*Nxy+(j)*Nxh+(i)]
#define alpha2(i,j,k) pSPARC->pois_const_stress2[(k)*Nxy+(j)*Nxh+(i)]
#define beta(i,j,k) pSPARC->pois_const_press[(k)*Nxy+(j)*Nxh+(i)]
    ...
    if (pSPARC->Calc_stress == 1) {
        pSPARC->pois_const_stress = (double *)malloc(sizeof(double) * Ndc);
        assert(pSPARC->pois_const_stress != NULL);
        if (pSPARC->ExxDivFlag == 0) {
            pSPARC->pois_const_stress2 = (double *)malloc(sizeof(double) * Ndc);
            assert(pSPARC->pois_const_stress2 != NULL);
        }
    } else if (pSPARC->Calc_pres == 1) {
        if (pSPARC->ExxDivFlag != 0) {
            pSPARC->pois_const_press = (double *)malloc(sizeof(double) * Ndc);
            assert(pSPARC->pois_const_press != NULL);
        }
    }
    /********************************************************************/
...
#define alpha(i,j,k,l) pSPARC->pois_const[(l)*Nd+(k)*Nxy+(j)*Nx+(i)]
#define alpha1(i,j,k,l) pSPARC->pois_const_stress[(l)*Nd+(k)*Nxy+(j)*Nx+(i)]
#define alpha2(i,j,k,l) pSPARC->pois_const_stress2[(l)*Nd+(k)*Nxy+(j)*Nx+(i)]
#define beta(i,j,k,l) pSPARC->pois_const_press[(l)*Nd+(k)*Nxy+(j)*Nx+(i)]
    ...
    pSPARC->pois_const = (double *)malloc(sizeof(double) * Nd * Nkpts_shift);
    assert(pSPARC->pois_const != NULL);
    // allocate space for stress
    if (pSPARC->Calc_stress == 1) {
        pSPARC->pois_const_stress = (double *)malloc(sizeof(double) * Nd * Nkpts_shift);
        assert(pSPARC->pois_const_stress != NULL);
        if (pSPARC->ExxDivFlag == 0) {
            pSPARC->pois_const_stress2 = (double *)malloc(sizeof(double) * Nd * Nkpts_shift);
            assert(pSPARC->pois_const_stress2 != NULL);
        }
    } else if (pSPARC->Calc_pres == 1) {
        if (pSPARC->ExxDivFlag != 0) {
            pSPARC->pois_const_press = (double *)malloc(sizeof(double) * Nd * Nkpts_shift);
            assert(pSPARC->pois_const_press != NULL);
        }
    }
    /********************************************************************/
