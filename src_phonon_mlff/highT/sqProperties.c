...
#define s_nl(i,j) s_nl[3*i+j]
    ...
    for (int nd = 0; nd < DMnd; nd++) {
        ...
        if (pSPARC->usefock) {
            ...
            Gauss_density_matrix_col(pSPARC, pSQ->Nd_loc, pSPARC->SQ_npl_g, DMcol_loc, pSQ->lanczos_vec, pSQ->w, pSQ->gnd[nd]);

            restrict_to_subgrid(DMcol_loc, DMcol, Nx_ex, Nx_loc, NxNy_ex, NxNy_loc, 
                FDn, Nx_loc+FDn-1, FDn, Ny_loc+FDn-1, FDn, Nz_loc+FDn-1, 0, 0, 0, sizeof(double));
        } else {
            // Already saved
            Gauss_density_matrix_col(pSPARC, pSQ->Nd_loc, pSPARC->SQ_npl_g, DMcol_loc, pSQ->lanczos_vec_all[nd], pSQ->w_all[nd], pSQ->gnd[nd]);
            restrict_to_subgrid(DMcol_loc, DMcol, Nx_ex, Nx_loc, NxNy_ex, NxNy_loc, 
                FDn, Nx_loc+FDn-1, FDn, Ny_loc+FDn-1, FDn, Nz_loc+FDn-1, 0, 0, 0, sizeof(double));
        }
        ...
        if(pSPARC->Calc_stress == 1) {
            ...
            // y-direction
            s_k(1,0) += 2* D1_1point(gradx_DM, center, pSPARC->D1_stencil_coeffs_y, FDn, Nx_loc) * pSPARC->dV;
            s_k(1,1) += 2* D1_1point(grady_DM, center, pSPARC->D1_stencil_coeffs_y, FDn, Nx_loc) * pSPARC->dV;
            s_k(1,2) += 2* D1_1point(gradz_DM, center, pSPARC->D1_stencil_coeffs_y, FDn, Nx_loc) * pSPARC->dV;

            // z-direction
            s_k(2,0) += 2* D1_1point(gradx_DM, center, pSPARC->D1_stencil_coeffs_z, FDn, NxNy_loc) * pSPARC->dV;
            s_k(2,1) += 2* D1_1point(grady_DM, center, pSPARC->D1_stencil_coeffs_z, FDn, NxNy_loc) * pSPARC->dV;
            s_k(2,2) += 2* D1_1point(gradz_DM, center, pSPARC->D1_stencil_coeffs_z, FDn, NxNy_loc) * pSPARC->dV;
        }
