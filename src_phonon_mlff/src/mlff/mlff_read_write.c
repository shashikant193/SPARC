...
void print_ref_atom_MLFF(MLFF_Obj *mlff_str){
    ...
    for (int j=0; j < mlff_str->natm_train_total; j++){
        ...
        for (int k=0; k < mlff_str->size_X2; k++){
            fprintf(fptr,"%.15f ",mlff_str->X2_traindataset[j][k]);
        }
        fprintf(fptr,"\n");
...
void read_MLFF_files(MLFF_Obj *mlff_str, SPARC_OBJ *pSPARC){
    ...
    while (!feof(fptr)){
        ...
        for (int i=0; i < pSPARC->Ntypes; i++){
            ...
            //    mlff_str->K_train = (double **)malloc(sizeof(double*)*mlff_str->n_rows);
            //    for (int m=0; m < mlff_str->n_rows; m++){
            //      mlff_str->K_train[m] = (double *)malloc(sizeof(double)*mlff_str->n_cols);
            //    }
            ...
            //    mlff_str->b_no_norm = (double *) malloc(sizeof(double)*mlff_str->n_rows);
            //     for (int i = 0; i < mlff_str->n_rows; i++){
            //        fscanf(fptr,"%lf", &K_elem);
            ...
            // mlff_str->weights = (double *) malloc(sizeof(double)*mlff_str->n_cols);
            for (int i = 0; i < mlff_str->n_cols; i++){
                fscanf(fptr,"%lf", &K_elem);
...
{  
   ...
   int str_no;
    ...
    while (!feof(fptr)){
        ...
        // fprintf(fpx,"structure_no: %d\n",str_no);
        // fprintf(fpx,"CELL:\n");
        // fprintf(fpx,"%lf %lf %lf\n",cell_data[str_no][0],cell_data[str_no][1],cell_data[str_no][2]);
        // fprintf(fpx,"LatUVec:\n");
        // fprintf(fpx,"%lf %lf %lf\n",LatUVec_data[str_no][0],LatUVec_data[str_no][1],LatUVec_data[str_no][2]);
        // fprintf(fpx,"%lf %lf %lf\n",LatUVec_data[str_no][3],LatUVec_data[str_no][4],LatUVec_data[str_no][5]);
         //fprintf(fpx,"%lf %lf %lf\n",LatUVec_data[str_no][6],LatUVec_data[str_no][7],LatUVec_data[str_no][8]);
        ...
        for (int i=0; i < nelem; i++){
         ...
         //fprintf(fpx,"natom_elem:\n");
         //fprintf(fpx,"%d\n%d\n",natom_elem_data[str_no][0], natom_elem_data[str_no][1]);

         //fprintf(fpx,"Atom_positions:\n");
        ...
        for (int i=0; i < natom_data[str_no]; i++){
            ...
            fscanf(fptr, "%*[^\n]\n");
             //fprintf(fpx,"%lf %lf %lf\n",apos_data[str_no][3*i], apos_data[str_no][3*i+1], apos_data[str_no][3*i+2]);
        }
        ...
        for (int i=0; i < natom_data[str_no]; i++){
            ...
            fscanf(fptr, "%*[^\n]\n");
            // fprintf(fpx,"%lf %lf %lf\n",F_data[str_no][3*i], F_data[str_no][3*i+1], F_data[str_no][3*i+2]);
        }
        fscanf(fptr,"%s",str);
         //fprintf(fpx,"Stress(GPa)\n");
        ...
        fscanf(fptr, "%*[^\n]\n");
			//fprintf(fpx,"%lf %lf %lf %lf %lf %lf\n",stress_data[str_no][0],stress_data[str_no][1],stress_data[str_no][2],stress_data[str_no][3],stress_data[str_no][4],stress_data[str_no][5]);
        if (str_no == nstr-1){
            break;
        }
    } 
