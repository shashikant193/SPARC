...
void init_exx_SQ(SPARC_OBJ *pSPARC)
{
    SQ_OBJ *pSQ = pSPARC->pSQ;
    if (pSQ->dmcomm_SQ == MPI_COMM_NULL) return;
    int rank;
    ...
    init_kron_Lap(pSPARC, Nx, Ny, Nz, 1, 1, 1, pSQ->kron_lap);
    ...
    if (pSPARC->ExxAcc == 1) {
        ...
        int nExxPot = (pSPARC->SQ_gauss_hybrid_mem == 1) ? pSQ->DMnd_SQ : 1;
