    ...
    int band_start_indx_source, double *stress_exx)
{
    ...
    for (loop = 0; loop < NL; loop ++) {
        ...
        poissonSolve(pSPARC, rhs, pSPARC->pois_const_stress, count-base, DMnd, dims, phi, comm);
        ...
        if (mflag == 0) {
            poissonSolve(pSPARC, rhs, pSPARC->pois_const_stress2, count-base, DMnd, dims, phi, comm);
            for (count = base; count < min(batch_num_rhs*(loop+1),num_rhs); count++) {
                i = rhs_list_i[count];
                j = rhs_list_j[count];
    ...
    double _Complex *psi, int ldp, double *occ, int Nband_source, int band_start_indx_source, int kpt_q, double *stress_exx)
{
    ...
    for (loop = 0; loop < NL; loop ++) {
        ...
        // Solve all Poisson's equation 
        poissonSolve_kpt(pSPARC, rhs, pSPARC->pois_const_stress, count-base, DMnd, dims, phi, kpt_k_list, kpt_q_list, comm);
        ...
        // additional term for spherical truncation
        if (mflag == 0) {
            poissonSolve_kpt(pSPARC, rhs, pSPARC->pois_const_stress2, count-base, DMnd, dims, phi, kpt_k_list, kpt_q_list, comm);
