...
double Calculate_occupation(SPARC_OBJ *pSPARC, double x1, double x2, double tol, int max_iter)
{
    ...
    Nt = pSPARC->CS_Flag == 1 ? pSPARC->CS_Nt : 0;
    Ns = pSPARC->Nstates;
    ...
    } else if (pSPARC->ext_FPMD_Flag != 0) {
        Efermi = Calculate_FermiLevel(pSPARC, x1, x2, tol, max_iter, occ_constraint_extFPMD);
    } else if (pSPARC->spDFT_Flag == 1) {
		Efermi = Calculate_FermiLevel(pSPARC, x1, x2, tol, max_iter, occ_constraint_spDFT);
		#ifdef DEBUG
        	t1 = MPI_Wtime();
		#endif

	} else {
        // Efermi = Calculate_FermiLevel(pSPARC, x1, x2, tol, max_iter, occ_constr`aint);
    ...
    // find occupations
    MPI_Bcast(&Efermi, 1, MPI_DOUBLE, 0, pSPARC->kptcomm); // added because of DDBP, check if can be removed 

    if (pSPARC->spDFT_Flag == 0){
    	// find occupations
		...
		spDFT_occupation(pSPARC, Efermi);
	}
