...
void Subspace_Rotation_DDBP(
    ...
    int *descX, MPI_Comm comm, int proc_active
)
{
    if (proc_active == 0) return;
    ...
    int **desc_psi, const int nkpt, const int kpt, const int spn_i)
{
    ...
    int proc_active = (DDBP_info->is_active_rowcomm) && (DDBP_info->elemcomm_index >= 0);
    ...
    for (int k = 0; k < nelem; k++) {
        ...
        DDBP_Element_Basis_Coeff_Multiply(
            ...
            psi_k, desc_psi_k, DDBP_info->rowcomm, proc_active
        );
    }
}
