...
void Setup_Comms_SQ(SPARC_OBJ *pSPARC) {
    ...
    // TODO: add input option to control the domain size like in SPARC parallelization
    ...
    if (pSPARC->dmcomm_phi != MPI_COMM_NULL) {
        ...
        // allocate memory for local part of XC potential (need for NLCC with GGA)
        pSPARC->v_xc_loc = (double *)malloc( DMnd * pSPARC->Nspin * sizeof(double) );
        assert(pSPARC->v_xc_loc != NULL); 
...
void create_D2D_sq2phi(SPARC_OBJ *pSPARC) {
    ...
    // sent by dmcomm_SQ
    sdims[0] = pSQ->npNdx_SQ;
    sdims[1] = pSQ->npNdy_SQ;
    sdims[2] = pSQ->npNdz_SQ;
...
void TransferDensity_sq2phi(SPARC_OBJ *pSPARC, double *rho_send, double *rho_recv) {
    ...
    sdims[0] = pSQ->npNdx_SQ; sdims[1] = pSQ->npNdy_SQ; sdims[2] = pSQ->npNdz_SQ;
...
void TransferVeff_phi2sq(SPARC_OBJ *pSPARC, double *Veff_send, double *Veff_recv) {
    ...
    rdims[0] = pSQ->npNdx_SQ; rdims[1] = pSQ->npNdy_SQ; rdims[2] = pSQ->npNdz_SQ;
    gridsizes[0] = pSPARC->Nx; gridsizes[1] = pSPARC->Ny; gridsizes[2] = pSPARC->Nz;
