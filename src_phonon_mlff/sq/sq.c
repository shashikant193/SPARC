...
void init_SQ(SPARC_OBJ *pSPARC) {
    ...
    pSQ->rem[0] = pSPARC->Nx % pSQ->npNdx_SQ;
    pSQ->rem[1] = pSPARC->Ny % pSQ->npNdy_SQ;
    pSQ->rem[2] = pSPARC->Nz % pSQ->npNdz_SQ;

    int DMnxnynz[3] = {DMnx, DMny, DMnz};
    int npNd[3] = {pSQ->npNdx_SQ, pSQ->npNdy_SQ, pSQ->npNdz_SQ};
...
/**
 * @brief   Write the initialized SQ parameters into the output file.
 */
void write_output_init_SQ(SPARC_OBJ *pSPARC, FILE *output_fp) {
    int i, j, nproc, count;
    ...
    fprintf(output_fp,"***************************************************************************\n");
    fprintf(output_fp,"Lattice (scaled) vectors:\n");
    ...
    // Use the output below after adding correct memory estimation
    // char mem_str[32];
