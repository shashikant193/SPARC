...
double soap_kernel_polynomial(double *X2_str, double *X3_str, double *X2_tr, double *X3_tr,
	...
	// kernel_val = norm_X3_str*norm_X3_tr;
	return kernel_val;
}
...
double der_soap_kernel_polynomial(double *dX2_str, double *dX3_str, double *X2_str, double *X3_str, double *X2_tr, double *X3_tr,
	...
	// der_val = (xi_3)*pow(dotProduct(X3_str, X3_tr, size_X3), xi_3-1)*dotProduct(X3_tr, dX3_str, size_X3);
	return der_val;
}

// double der_soap_kernel_polynomial(double *dX2_str, double *dX3_str, double *X2_str, double *X3_str, double *X2_tr, double *X3_tr_d,
// 			 double beta_2, double beta_3, double xi_3, int size_X2, int size_X3) {

// 	double dot_X3str_X3str = 0.0;
// 	double dot_X3tr_X3tr = 0.0;
// 	double dot_X3str_X3tr = 0.0;
// 	for(int i = 0; i < size_X3; i++){
// 		dot_X3str_X3str += X3_str[i] * X3_str[i];
// 		dot_X3tr_X3tr += X3_tr_d[i] * X3_tr_d[i];
// 		dot_X3str_X3tr += X3_str[i] * X3_tr_d[i];
// 	}
// 	double norm_X3_str = sqrt(dot_X3str_X3str);
// 	double norm_X3_tr = sqrt(dot_X3tr_X3tr);

// 	double kernel = dot_X3str_X3tr/(norm_X3_str*norm_X3_tr);
// 	kernel = pow(kernel, xi_3);

// 	double W[size_X3], W1[size_X3], W2[size_X3];
// 	for (int i = 0; i < size_X3; i++){
// 		W1[i] = X3_str[i]/dot_X3str_X3str ;
// 		W2[i] = X3_tr_d[i]/dot_X3str_X3tr;
// 		W[i] = W1[i]-W2[i] ;
// 	}
// 	double der_val = -1.0*xi_3 * kernel * dotProduct(W, dX3_str, size_X3);
// 	der_val = dotProduct(X3_str, dX3_str, size_X3) * norm_X3_tr/norm_X3_str;
// 	return der_val;
// }
...
{
    int i;
    double complex vec_dot = 0.0+0.0*I;
    for (i = 0; i < len; i++){
...
double complex der2_soap_kernel_polynomial(double complex *dX3_str_alpha, double complex *dX3_str_beta,  double complex *d2X3_str, double complex *X3_str, double *X3_tr_d,
	...
	double dot_X3str_X3tr = 0.0;
	for(int i = 0; i < size_X3; i++){
		dot_X3str_X3str += creal(X3_str[i]) * creal(X3_str[i]);
		dot_X3tr_X3tr += X3_tr_d[i] * X3_tr_d[i];
		dot_X3str_X3tr += creal(X3_str[i]) * X3_tr_d[i];
	}
	double norm_X3_str = sqrt(dot_X3str_X3str);
	double norm_X3_tr = sqrt(dot_X3tr_X3tr);
	...
	// double W[size_X3], W1[size_X3], W2[size_X3];
	double *W = (double *) malloc(sizeof(double)*size_X3);
	double *W1 = (double *) malloc(sizeof(double)*size_X3);
	double *W2 = (double *) malloc(sizeof(double)*size_X3);
	...
	double complex dot_dX_W1 = 0.0+0.0*I;
	double complex dot_dX_W2 = 0.0+0.0*I;
	for (int i = 0; i < size_X3; i++){
		dot_dX_W1 += dX3_str_alpha[i] * W1[i];
		dot_dX_W2 += dX3_str_alpha[i] * W2[i];
	}

	// double complex dW[size_X3];
	double complex *dW = (double complex *)malloc(sizeof(double complex)*size_X3);
	for (int i = 0; i < size_X3; i++){
		dW[i] = (1.0/dot_X3str_X3str) * dX3_str_alpha[i] - 2.0*W1[i]*dot_dX_W1 + W2[i]*dot_dX_W2;
	}

	double complex dot_d2X3_W = 0.0+0.0*I;
	double complex dot_dX3B_dW = 0.0+0.0*I;
	double complex dot_dX3B_W = 0.0+0.0*I;
	double complex dot_dX3A_W = 0.0+0.0*I;

	for(int i = 0; i < size_X3; i++){
		double complex dX3_str_beta_conj = conj(dX3_str_beta[i]);
		dot_d2X3_W +=  d2X3_str[i] * W[i];
		dot_dX3B_dW += dX3_str_beta_conj * dW[i];
		dot_dX3B_W += dX3_str_beta_conj * W[i];
		dot_dX3A_W += dX3_str_alpha[i] * W[i];
	}

	double complex der2_val = -1.0 * (xi_3 * kernel *  (dot_d2X3_W + dot_dX3B_dW - xi_3 * dot_dX3B_W * dot_dX3A_W));
	// double complex der2_val = -1.0 * (xi_3 * kernel *  (dot_dX3B_dW ));

	// double complex der2_val = 0.0+0.0*I;
	// for (int i = 0; i < size_X3; i++){
	// 	der2_val = der2_val + 
	// }
...
void calculate_Kpredict(SoapObj *soap_str, NeighList *nlist, MLFF_Obj *mlff_str, double **K_predict){
 ...
 //    FILE *fp2, *fp3;
	// fp2 = fopen("Kmat_F.txt","w");
	// fp3 = fopen("Kmat_E.txt","w");
	// for (int j = 0; j < mlff_str->n_cols; j++){
	// 	fprintf(fp3, "%.15f ", (1.0/temp)*K_predict[0][j]);
	// }
	// // fprintf(fp, "\n");
	// fclose(fp3);
	...
	// for (int i = 0; i < soap_str->natom_domain; i++){
	// 	for (int k = 0; k < 3; k++){
	// 		for (int l =0; l < mlff_str->n_cols; l++){
	// 			fprintf(fp2, "%.15f ", (1.0/ F_scale)*K_predict[3*i+1+k+mlff_str->stress_len][l]);
	// 		}
	// 		fprintf(fp2, "\n");
	// 	}
	// }
 ...
 void calculate_Kpredict_phonon(SoapObj *soap_str, NeighList *nlist, MLFF_Obj *mlff_str, double complex **dyn_predict, double complex *weights){
    ...
    MPI_Comm_size(MPI_COMM_WORLD, &nproc);
 	...
 	double dyn_scale = mlff_str->std_E;


 	int rows = 9*soap_str->natom*soap_str->natom;
    int cols = mlff_str->n_cols;
    double complex alpha1 = 1.0+0.0*I, beta1 = 0.0+0.0*I;
	...
	// double temp = (1.0/ (double) soap_str->natom);
	double complex *K_predict_local = (double complex*) malloc(sizeof(double complex)*mlff_str->n_cols*(9*natom*natom)); // row major;
	...
	for (int q = 0; q < mlff_str->Nq; q++){
		...
		for (int i=0; i < mlff_str->n_cols*(9*natom*natom); i++){
			K_predict_local[i] = 0.0+0.0*I;
		}

		t1 = MPI_Wtime();
		int count_der2_call=0;
		for (int i=0; i <soap_str->natom_domain; i++){
			el_type = soap_str->el_idx_domain[i];
			atm_idx = soap_str->atom_idx_domain[i];
			for (int iatm_el_tr = 0; iatm_el_tr < mlff_str->natm_train_elemwise[el_type]; iatm_el_tr++){
				...
				for (int alpha = 0; alpha < 3; alpha++){
					for (int beta = 0; beta < 3; beta++){
						int idx = (beta + 3*d) + (alpha+3*c)*3*soap_str->natom;

						K_predict_local[idx*mlff_str->n_cols + col_idx] +=  dyn_scale*der2_soap_kernel_polynomial( 
							soap_str->dX3_q[q][alpha][i][0], soap_str->dX3_q[q][beta][i][0], soap_str->d2X3_q[q][3*alpha+beta][i][0][0],
							soap_str->X3_q[q][i], mlff_str->X3_traindataset[col_idx], xi_3, size_X3);
						count_der2_call+=1;
					}
				}
				...
				for (int neighs1 =0; neighs1 < soap_str->unique_Nneighbors[i]; neighs1++){
					c = soap_str->unique_neighborList[i].array[neighs1];
					d = atm_idx;
					for (int alpha = 0; alpha < 3; alpha++){
						for (int beta = 0; beta < 3; beta++){
							int idx = (beta + 3*d) + (alpha+3*c)*3*soap_str->natom;
							...
							K_predict_local[idx*mlff_str->n_cols + col_idx] +=  dyn_scale*der2_soap_kernel_polynomial( 
								soap_str->dX3_q[q][alpha][i][1+neighs1], soap_str->dX3_q[q][beta][i][0], soap_str->d2X3_q[q][3*alpha+beta][i][1+neighs1][0],
								soap_str->X3_q[q][i], mlff_str->X3_traindataset[col_idx], xi_3, size_X3);
							count_der2_call+=1;
						}
					}
					...
					d = soap_str->unique_neighborList[i].array[neighs1];
					c = atm_idx;
					for (int alpha = 0; alpha < 3; alpha++){
						for (int beta = 0; beta < 3; beta++){
							int idx = (beta + 3*d) + (alpha+3*c)*3*soap_str->natom;
							...
							K_predict_local[idx*mlff_str->n_cols + col_idx] +=  dyn_scale*der2_soap_kernel_polynomial( 
								soap_str->dX3_q[q][alpha][i][0], soap_str->dX3_q[q][beta][i][1+neighs1], soap_str->d2X3_q[q][3*alpha+beta][i][0][1+neighs1],
								soap_str->X3_q[q][i], mlff_str->X3_traindataset[col_idx], xi_3, size_X3);
							count_der2_call+=1;
						}
					}

					for (int neighs2 =0; neighs2 < soap_str->unique_Nneighbors[i]; neighs2++){
						c = soap_str->unique_neighborList[i].array[neighs1];
						d = soap_str->unique_neighborList[i].array[neighs2];
						...
						for (int alpha = 0; alpha < 3; alpha++){
							for (int beta = 0; beta < 3; beta++){
								...
								K_predict_local[idx*mlff_str->n_cols + col_idx] +=  dyn_scale*der2_soap_kernel_polynomial( 
									soap_str->dX3_q[q][alpha][i][1+neighs1], soap_str->dX3_q[q][beta][i][1+neighs2], soap_str->d2X3_q[q][3*alpha+beta][i][1+neighs1][1+neighs2],
									soap_str->X3_q[q][i], mlff_str->X3_traindataset[col_idx], xi_3, size_X3);
								count_der2_call+=1;
							}
						}
		...
		t2 = MPI_Wtime();
		if ( rank ==0){
	        fprintf(fp_mlff, "Loop in Kpredict done. Time taken: %.3f s, count_der2call: %d\n", t2-t1, count_der2_call);
	    }
	    t1 = MPI_Wtime();

		cblas_zgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, 
                rows, 1, cols, &alpha1, K_predict_local, cols, weights, 1,  &beta1, dyn_predict[q], 1);
		t2 = MPI_Wtime();
		if ( rank ==0){
	        fprintf(fp_mlff, "cblas_zgemm done. Time taken: %.3f s\n", t2-t1);
	    }
	}

	free(K_predict_local);

		
	t1 = MPI_Wtime();
	for (int q = 0; q < mlff_str->Nq; q++){
		// MPI_Allreduce(dyn_predict[q], dyn_predict[q], (9*natom*natom), MPI_C_DOUBLE_COMPLEX, MPI_SUM, MPI_COMM_WORLD);
		// MPI_Allreduce(MPI_IN_PLACE, dyn_predict[q], 9*natom*natom, MPI_C_DOUBLE_COMPLEX, MPI_SUM, MPI_COMM_WORLD);
		if (rank ==0){
			MPI_Reduce(MPI_IN_PLACE, dyn_predict[q], 9*natom*natom, MPI_C_DOUBLE_COMPLEX, MPI_SUM, 0, MPI_COMM_WORLD);
		} else {
			MPI_Reduce(dyn_predict[q], NULL, 9*natom*natom, MPI_C_DOUBLE_COMPLEX, MPI_SUM, 0, MPI_COMM_WORLD);
		}
		
	}
	t2 = MPI_Wtime();
	if ( rank ==0){
        fprintf(fp_mlff, "MPI_Reduce done. Time taken: %.3f s\n", t2-t1);
    }
