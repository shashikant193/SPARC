...
void build_nlist(const double rcut, const int nelem, const int natom, const double * const atompos,
	...
	double rcut_x = rcut;
	double rcut_y = rcut * geometric_ratio[0]*2;
	double rcut_z = rcut * geometric_ratio[1]*2;
					...
					for (int m=0; m < l+1; m++){
						...
						if (atm_idx != idx_neigh){ // all neighbors except self-images
								...
								if ((fabs(dtheta) < temp_tol || fabs(dtheta - M_PI) < temp_tol)){
									...
									// if (i == 0 && local_index ==10){
									// 	printf("%.15f %.15f %d %d %d 	%.15f %.15f 	%.15f %.15f\n", n, l, m, dtheta, dphi, creal(Ylm_val), cimag(Ylm_val), creal(dYlm_theta_val *(fact/dr)), cimag(dYlm_theta_val *(fact/dr)));
									// }
