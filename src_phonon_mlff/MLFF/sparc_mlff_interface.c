...
void init_MLFF(SPARC_OBJ *pSPARC) {
	...
	mlff_str->mlff_phonon_flag = pSPARC->mlff_phonon_flag;

	mlff_str->Nq = pSPARC->Nkpts_kptcomm;
	FILE *fp1;

	if (rank==0) 	printf("Nq: %d\n", mlff_str->Nq);
	mlff_str->qvec = (double *) calloc(3*mlff_str->Nq, sizeof(double));
	if (rank==0) fp1 = fopen("print_qvec.txt","w");
	double kpt_vec[3];
	for (int kpt = 0; kpt < pSPARC->Nkpts_kptcomm; kpt++){
	    ...
	    mlff_str->qvec[3*kpt] = kpt_vec[0];
	    mlff_str->qvec[3*kpt+1] = kpt_vec[1];
	    mlff_str->qvec[3*kpt+2] = kpt_vec[2];
	   if (rank==0){

	   	fprintf(fp1, "qvec: %.15f %.15f %.15f\n",
	   		 mlff_str->qvec[3*kpt], mlff_str->qvec[3*kpt+1], mlff_str->qvec[3*kpt+2]);
	   } 
	}
	if (rank==0) fclose(fp1);
 ...
 //    if(rank==0){
 //    	// Open the file
	//     file = fopen("qvec.txt", "r");

	//     // Read the number of rows from the top of the file
	//     if (fscanf(file, "%d", &numRows) != 1) {
	//         perror("Error reading the number of rows");
	//         fclose(file);
	//     }

	//     // Allocate memory for the columns
	//     col1 = (double *)malloc(numRows * sizeof(double));
	//     col2 = (double *)malloc(numRows * sizeof(double));
	//     col3 = (double *)malloc(numRows * sizeof(double));
	...
	//         if (fscanf(file, "%lf %lf %lf", &col1[i], &col2[i], &col3[i]) != 3) {
 ...
 //    mlff_str->Nq = numRows;
	...
	// 	mlff_str->qvec[3*i] = col1[i];
	// 	mlff_str->qvec[3*i+1] = col2[i];
	// 	mlff_str->qvec[3*i+2] = col3[i];
	// }
 ...
 //    free(col3);
	...
	if (rank==0){
		...
		mlff_str->d2h_nl = (double *) malloc(sizeof(double)* pSPARC->N_rgrid_MLFF* pSPARC->N_max_SOAP*(pSPARC->L_max_SOAP+1));
		read_h_nl(pSPARC->N_max_SOAP, pSPARC->L_max_SOAP, mlff_str->rgrid, mlff_str->h_nl, mlff_str->dh_nl, mlff_str->d2h_nl, pSPARC);
		...
		MPI_Bcast(mlff_str->d2h_nl, pSPARC->N_rgrid_MLFF* pSPARC->N_max_SOAP*(pSPARC->L_max_SOAP+1), MPI_DOUBLE, 0, MPI_COMM_WORLD);
	} else {
		MPI_Bcast(&pSPARC->N_rgrid_MLFF, 1, MPI_INT, 0, MPI_COMM_WORLD);
		mlff_str->rgrid = (double *) malloc(sizeof(double)* pSPARC->N_rgrid_MLFF);
	 	mlff_str->h_nl = (double *) malloc(sizeof(double)* pSPARC->N_rgrid_MLFF* pSPARC->N_max_SOAP*(pSPARC->L_max_SOAP+1));
...
t2 = MPI_Wtime();
	...
	if (pSPARC->mlff_phonon_flag == 1){
		calculate_phonon(pSPARC, mlff_str);
	}
